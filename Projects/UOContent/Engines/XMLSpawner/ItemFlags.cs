...
namespace Server.Items
{
    public class ItemFlags
    {
        ...
        public static void SetStealable(Item target, bool value)
        {
            if (target != null)
            {
                target.SetSavedFlag((int)CompactInfoFlag.StealableFlag, value);
            }
        }
        ...
        public static bool GetStealable(Item target)
        {
            if (target != null)
            {
                return target.GetSavedFlag((int)CompactInfoFlag.StealableFlag);
            }
            else
            {
                return false;
        ...
        public static void SetTaken(Item target, bool value)
        {
            if (target != null)
            {
                target.SetSavedFlag((int)CompactInfoFlag.TakenFlag, value);
            }
        }
        ...
        public static bool GetTaken(Item target)
        {
            if (target != null)
            {
                return target.GetSavedFlag((int)CompactInfoFlag.TakenFlag);
            }
            else
            {
                return false;
        ...
        public static void GetFlag_OnCommand(CommandEventArgs e)
        {
            ...
            {
                if (e.Arguments[0].StartsWith("0x", StringComparison.Ordinal))
                {
