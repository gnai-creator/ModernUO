...
{
    [TypeAlias("Server.Mobiles.Spawner", "Server.Mobiles.PremiumSpawner")]
    public class XmlSpawner : Item, ISpawner, ISpawnerItem, ISpawnObjectFinderList
    {
        ...
        public const byte MaxLoops = 5; //maximum number of recursive calls from spawner to itself. this is to prevent stack overflow from xmlspawner scripting
        ...
        private const string XmlDataSetName = "Spawns";
        public static AccessLevel DiskAccessLevel = AccessLevel.Seer; // minimum access level required by commands that can access the disk such as XmlLoad, XmlSave, and the Save function of XmlEdit
        public const AccessLevel ConfigAccessLevel = AccessLevel.GameMaster;
...
#endif
        private static int MaxMoveCheck = 5; // limit number of players that can be checked for triggering in a single OnMovement tick
        ...
        private static float defTriggerProbability = 1;
        ...
        private static Dictionary<Sector, HashSet<XmlSpawner>>[] GlobalSectorTable = new Dictionary<Sector, HashSet<XmlSpawner>>[6];
        ...
        public List<ISpawnObjectFinder> i_SpawnObjects => m_SpawnObjects.ToList<ISpawnObjectFinder>();
        ...
        private bool m_ExternalManualTrigger;
        ...
        public static Dictionary<string, XmlSpawner> RecentSpawnerSearch = new Dictionary<string, XmlSpawner>();
        public static Dictionary<string, Item> RecentItemNameSearch = new Dictionary<string, Item>();
        public static Dictionary<(string, Type), Item> RecentItemNameTypeSearch = new Dictionary<(string, Type), Item>();
        public static Dictionary<string, Mobile> RecentMobileNameSearch = new Dictionary<string, Mobile>();
        public static Dictionary<(string, Type), Mobile> RecentMobileNameTypeSearch = new Dictionary<(string, Type), Mobile>();
        ...
        private float m_SkillTriggerMin;
        private float m_SkillTriggerMax;
        ...
        public static TimerPriority BasePriority { get; } = TimerPriority.OneSecond;
        ...
        public Point3D MostRecentSpawnPosition { get; set; } = Point3D.Zero;
        ...
        public TimeSpan GameTOD
        {
            get
            {
                ...
                Server.Items.Clock.GetTime(Map, Location.X, Location.Y, out int hours, out int minutes);
		...
		*/
                    ...
                    for (int x = x1; x <= x1 + width; x += Server.Map.SectorSize)
                    {
                        for (int y = y1; y <= y1 + height; y += Server.Map.SectorSize)
                        {
                            ...
                            if (!duplicate)
                            {
                                ...
                                if (GlobalSectorTable[Map.MapID].TryGetValue(s, out HashSet<XmlSpawner> spawnerlist))//.Contains(s))
                                {
                                    //List<XmlSpawner> spawnerlist = GlobalSectorTable[Map.MapID][s];
                                    if (spawnerlist == null)
                                    {
                                        //GlobalSectorTable[Map.MapID].Remove(s);
                                        spawnerlist = new HashSet<XmlSpawner>();
                                ...
                                if (sectorList.Count > MaxSmartSectorListSize)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            op.WriteLine("{0} SmartSpawning disabled at {1} {2} : Range too large.", Core.MistedDateTime, loc, Map);
                                            op.WriteLine();
                                        }
                                    }
                                    catch
        ...
        public DateTime FirstModified { get; private set; }
            ...
            set
            {
                if (!string.IsNullOrEmpty(value))
                {
                    ...
                    else
                    {
                        ...
                        if (typestr != null && (BaseXmlSpawner.IsTypeOrItemKeyword(typestr) || typestr.IndexOf("{") != -1 || typestr.StartsWith("*", StringComparison.Ordinal) || typestr.StartsWith("#", StringComparison.Ordinal)))
                        {
                            m_SpawnObjects.Add(new XmlSpawner.SpawnObject(str, 1));
                        }
                        else
        ...
        public bool CanFreeSpawn
        {
            get
            {
                ...
                if (Running && m_ProximityRange == -1 &&
                    ...
                    (!ExternalTriggering || m_ExternalManualTrigger))
                {
                    return true;
                }
                else
        ...
        public List<SpawnObject> SpawnObjects
        {
            ...
            private set
            {
                AddSpawnObjects(value);
            }
        }


        public void AddSpawnObjects(List<SpawnObject> spawns, bool uselistspawns = false)
        {
            if(uselistspawns)
            {
                m_SpawnObjects.Clear();
                m_SpawnObjects.AddRange(spawns);
            }
            else
            {
                for(int i = 0; i < spawns.Count; ++i)
                {
                    SpawnObject so = spawns[i];
        ...
        public string RegionName
        {
            ...
            set
            {
                ...
                Map map = Map;
                if (RootParent != null)//dentro un contenitore o indossato?!
                {
                    map = RootParent.Map;
                }
                ...
                if (map.Regions.TryGetValue(m_RegionName, out Region region) && region != null)
                {
                    m_Region = region;
		...
		*/
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public bool ExtManualTrigger
        {
            get => m_ExternalManualTrigger;
            set
            {
                if (value)
                {
                    m_ExternalManualTrigger = true;
                    if (!Deleted)
                    {
                        Spawn(false, 0);
                    }
                    m_ExternalManualTrigger = false;
                }
            }
        }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public float TriggerProbability { get; set; } = defTriggerProbability;

        //added refractory period support
        [CommandProperty(AccessLevel.GameMaster)]
        public TimeSpan RefractMin { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public TimeSpan RefractMax { get; set; }
        ...
        public TimeSpan RefractoryOver
        {
            get
            {
                ...
                {
                    if (m_RefractoryTimer != null && m_RefractoryTimer.Running && m_RefractoryTimer.Next > DateTime.UtcNow)
                    {
                        return m_RefractoryTimer.Next.Subtract(DateTime.UtcNow);
                    }
                }
        ...
        public string TriggerObjectName
        {
            get
            {
                if (TriggerObject == null || TriggerObject.Deleted)
                {
                    return null;
                }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public TimeSpan TODStart { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public TimeSpan TODEnd { get; set; }
        [CommandProperty(AccessLevel.GameMaster)]
        public TimeSpan TOD
        {
            get
            {
                ...
                {
                    return Core.MistedDateTime.TimeOfDay;
                }
            }
        ...
        public bool TODInRange
        {
            get
            {
                ...
                {
                    Server.Items.Clock.GetTime(Map, Location.X, Location.Y, out int hours, out int minutes);
                    now = new DateTime(Core.MistedDateTime.Year, Core.MistedDateTime.Month, Core.MistedDateTime.Day, hours, minutes, 0);
                }
                else
                {
                    // calculate the time window
                    now = Core.MistedDateTime;
                }
                day_start = new DateTime(now.Year, now.Month, now.Day);
                // calculate the starting TOD window by adding the TODStart to day_start
                TOD_start = day_start + TODStart;
                TOD_end = day_start + TODEnd;
        ...
        public string SkillTrigger
        {
            ...
            set
            {
                ...
                if (value != null)
                {
                    ...
                    string[] arglist = BaseXmlSpawner.ParseString(value, 4, BaseXmlSpawner.CommaDelim);
                    ...
                    if (arglist.Length == 3)
                    {
                        ...
                        try
                        {
                            minval = float.Parse(arglist[1]);
                            maxval = float.Parse(arglist[2]);
                        }
                        catch { }
                    }
                    else
                        if (arglist.Length == 4)
                    {
                        try
                        {
                            minval = float.Parse(arglist[2]);
                            maxval = float.Parse(arglist[3]);
                        }
                        catch { }
                    }
                    try
        ...
        public bool LoadConfig
        {
            ...
            set
            {
                if (value == true)
                {
                    LoadXmlConfig(Path.Combine(XmlConfigsDir, ConfigFile));
                }
            }
        }
        ...
        public override void OnDoubleClick(Mobile from)
        {
            ...
            DeleteTextEntryBook();
            from.SendGump(new XmlSpawnerGump(this, 0, 0, 0, 0, 0));
        }
        ...
        private static XmlGumpCallback SpawnerCB_Instance = new XmlGumpCallback(SpawnerGumpCallback);
        private static void SpawnerGumpCallback(Mobile from, object invoker, string response)
        {
        ...
        public static int ConvertToInt(string value)
        {
            ...
            {
                int.TryParse(value.Substring(2), NumberStyles.HexNumber, Core.Culture, out int val);
                return val;
            }
            else
            {
        ...
        private static void RemoveFromSectorTable(Sector s, XmlSpawner spawner)
        {
            ...
            // find the sector
            if (GlobalSectorTable[s.Owner.MapID].TryGetValue(s, out HashSet<XmlSpawner> spawnerlist) && spawnerlist != null)
            {
                ...
                if (ds.Tables != null && ds.Tables.Count > 0)
                {
                    if (ds.Tables[XmlTableName] != null && ds.Tables[XmlTableName].Rows.Count > 0)
                    {
                        foreach (DataRow dr in ds.Tables[XmlTableName].Rows)
                        {
                            ...
                            valid_entry = true;
                            try { floatEntry = float.Parse((string)dr["MinDelay"]); }
                            catch { valid_entry = false; }
                            if (valid_entry)
                            {
                                if (delayinsec)
                                {
                                    m_MinDelay = TimeSpan.FromSeconds(floatEntry);
                                }
                                else
                                {
                                    m_MinDelay = TimeSpan.FromMinutes(floatEntry);
                                }
                            }

                            valid_entry = true;
                            try { floatEntry = float.Parse((string)dr["MaxDelay"]); }
                            catch { valid_entry = false; }
                            if (valid_entry)
                            {
                                if (delayinsec)
                                {
                                    m_MaxDelay = TimeSpan.FromSeconds(floatEntry);
                                }
                                else
                                {
                                    m_MaxDelay = TimeSpan.FromMinutes(floatEntry);
                                }
                            }

                            valid_entry = true;
                            try { floatEntry = float.Parse((string)dr["Duration"]); }
                            catch { valid_entry = false; }
                            if (valid_entry) { m_Duration = TimeSpan.FromMinutes(floatEntry); }

                            valid_entry = true;
                            try { floatEntry = float.Parse((string)dr["DespawnTime"]); }
                            catch { valid_entry = false; }
                            if (valid_entry) { DespawnTime = TimeSpan.FromHours(floatEntry); }

                            valid_entry = true;
                            try { floatEntry = float.Parse((string)dr["MinRefractory"]); }
                            catch { valid_entry = false; }
                            if (valid_entry) { RefractMin = TimeSpan.FromMinutes(floatEntry); }

                            valid_entry = true;
                            try { floatEntry = float.Parse((string)dr["MaxRefractory"]); }
                            catch { valid_entry = false; }
                            if (valid_entry) { RefractMax = TimeSpan.FromMinutes(floatEntry); }

                            valid_entry = true;
                            try { floatEntry = float.Parse((string)dr["TODStart"]); }
                            catch { valid_entry = false; }
                            if (valid_entry) { TODStart = TimeSpan.FromMinutes(floatEntry); }

                            valid_entry = true;
                            try { floatEntry = float.Parse((string)dr["TODEnd"]); }
                            catch { valid_entry = false; }
                            if (valid_entry) { TODEnd = TimeSpan.FromMinutes(floatEntry); }
                            ...
                            valid_entry = true;
                            try { floatEntry = float.Parse((string)dr["TriggerProbability"]); }
                            catch { valid_entry = false; }
                            if (valid_entry) { TriggerProbability = floatEntry; }
                            ...
                            List<SpawnObject> Spawns = new List<SpawnObject>();
                            bool havenew = true;
                            valid_entry = true;
        ...
        public void ReportStatus()
        {
            ...
            Console.WriteLine("Dungeon Semi-Chiusi");
            for (int i = 0; i < XmlSpawnerSkillCheck.RegisteredSkill.MaxSkills + 1; ++i)
            {

                if (XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.DungeonSemiChiusi).Count > 0)
                {
                    Console.WriteLine("\t{0} : {1}", (SkillName)i, XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.DungeonSemiChiusi).Count);
                }

                count += XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.DungeonSemiChiusi).Count;
            }

            Console.WriteLine("Dungeon Chiusi");
            for (int i = 0; i < XmlSpawnerSkillCheck.RegisteredSkill.MaxSkills + 1; ++i)
            {

                if (XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.DungeonChiusi).Count > 0)
                {
                    Console.WriteLine("\t{0} : {1}", (SkillName)i, XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.DungeonChiusi).Count);
                }

                count += XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.DungeonChiusi).Count;
            }
            ...
            for (int i = 0; i < XmlSpawnerSkillCheck.RegisteredSkill.MaxSkills + 1; ++i)
            {
                ...
                if (XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.Eventi).Count > 0)
                {
                    Console.WriteLine("\t{0} : {1}", (SkillName)i, XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.Eventi).Count);
                }

                count += XmlSpawnerSkillCheck.RegisteredSkill.TriggerList((SkillName)i, Map.Eventi).Count;
            }
...
public static void TraceEnd(int index)
{
	   if(index < MaxTraces){
        ...
        private void ActivateTrigger()
        {
            ...
            if (RefractMax > TimeSpan.FromMinutes(0))
            {
                DoTimer3(TimeSpan.FromSeconds(Utility.RandomDouble(RefractMin.TotalSeconds, RefractMax.TotalSeconds)));
            }
        ...
        public void CheckTriggers(Mobile m, Skill s, bool hasproximity)
        {
            ...
            if (AllowTriggering && !m_proximityActivated)
            {
                ...
                if (TriggerObjectProp != null && TriggerObjectProp.Length > 0)
                {
                    ...
                    if (BaseXmlSpawner.TestItemProperty(this, TriggerObject, TriggerObjectProp, null, out string status_str))
                    {
                ...
                if (PlayerTriggerProp != null && PlayerTriggerProp.Length > 0)
                {
                    ...
                    if (BaseXmlSpawner.TestMobProperty(this, m, PlayerTriggerProp, null, out string status_str))
                    {
                    ...
                    MobTriggerName != null && MobTriggerName.Length > 0)
                {
                    ...
                    if (BaseXmlSpawner.TestMobProperty(this, MobTriggerId, MobTriggerProp, null, out string status_str))
                    {
                ...
                if (!hasproximity && !needs_object_trigger && !needs_mob_trigger && !ExternalTriggering)
                {
                    return;
                }
                ...
                if (Utility.RandomFloat() < TriggerProbability)
                {
        ...
        public override void OnSpeech(SpeechEventArgs e)
        {
            if ( /*!e.Handled && */m_Running && m_ProximityRange >= 0 && ValidPlayerTrig(e.Mobile) && CanSpawn && !m_refractActivated && TODInRange)
            {
                ...
                /*if (SpeechTrigger != null && e.Speech.ToLower(Core.Culture).IndexOf(SpeechTrigger.ToLower(Core.Culture)) >= 0)
                {
                    e.Handled = true;
                ...
                if (SpeechTrigger != null)
                {
                    ...
                    if(!string.IsNullOrEmpty(keyword) && from != null)
                    {
                        string[] arglist = SpeechTrigger.Split(',', StringSplitOptions.RemoveEmptyEntries);
                        for (int i = 0; i < arglist.Length; ++i)
                        {
                            if (arglist[i] == "$TRIGNAME")
                            {
                                string[] tocheck = from.Name.ToLower(Core.Culture).Split(' ');
                                string result = string.Empty;

                                for (int x = 0; x < tocheck.Length; ++x)
                                {
                                    if (keyword.Contains(tocheck[x], StringComparison.OrdinalIgnoreCase))
                                    {
                                        result = tocheck[x];
                                    }
                                }
                            ...
                            {
                                // special match anything expression
                                e.Handled = true;
                            ...
                            {
                                if (keyword.Contains(arglist[i], StringComparison.OrdinalIgnoreCase))
                                {
        ...
        private static void DelayedAssignSettings((string, string) state)
        {
            string argname = state.Item1;
            string value = state.Item2;
            switch (argname)
            {
                case "BlockCommand":
                {
                    ...
                    if (keywordlist != null && keywordlist.Length > 0)
                    {
                        for (int i = 0; i < keywordlist.Length; ++i)
                        {
                            string commandname = keywordlist[i].Trim().ToLower(Core.Culture);
                            try
                            {
                ...
                case "ChangeCommand":
                {
                    ...
                    if (keywordlist != null && keywordlist.Length > 0)
                    {
                        for (int i = 0; i < keywordlist.Length; ++i)
                        {
                            ...
                            {
                                string oldname = namelist[0].Trim().ToLower(Core.Culture);
                                ...
                                foreach (BaseCommand b in TargetCommands.AllCommands)
                                {
                                    if (b.Commands != null)
                                    {
                                        for (int j = 0; j < b.Commands.Length; j++)
                                        {
                                            ...
                                            if (commandname.ToLower(Core.Culture) == oldname)
                                            {
        ...
        {
            EventSink.WorldPostLoad += new WorldPostLoadEventHandler(PostLoad);
        }
        ...
        private static void PostLoad()
        {
            ...
            //call all of this AFTER REAL LOAD, not per single load
            foreach (Item item in World.Items.Values)
            {
                if (item is XmlSpawner spawner)
                {
					...
					*/
                    ...
                    if (spawner.m_ShowBoundsItems != null)
                    {
                        // Remove all of the items from the array
                        foreach (Static s in spawner.m_ShowBoundsItems)
                        {
                            s.Delete();
                        }

                        spawner.m_ShowBoundsItems.Clear();
                    }
                }
            }
        ...
        public static void Initialize()
        {
            ...
            CommandSystem.Register("XmlGet", AccessLevel.Administrator, new CommandEventHandler(XmlGetValue_OnCommand));
        ...
        private class GetValueTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                string result = BaseXmlSpawner.GetPropertyValue(null, targeted, pname, out Type ptype);
        ...
        private static void LoadDefaults(XmlElement node)
        {
            ...
            try { XmlSpawner.defTriggerProbability = float.Parse(node["defTriggerProbability"].InnerText); }
            catch { }
        ...
        public static void XmlDefaults_OnCommand(CommandEventArgs e)
        {
            ...
            if (e.Arguments.Length >= 1)
            {
                ...
                {
                    if (e.Arguments[0].ToLower(Core.Culture) == "save")
                    {
                        XmlSaveDefaults(e.Arguments[1], m);
                    }
                    else
                        if (e.Arguments[0].ToLower(Core.Culture) == "load")
                    {
                        XmlLoadDefaults(e.Arguments[1], m);
                    }
                    else
                            // try to set the property
                            if (e.Arguments[0].ToLower(Core.Culture) == "maxdelay")
                    {
                        try
                        {
                            XmlSpawner.defMaxDelay = TimeSpan.FromMinutes(Convert.ToSingle(e.Arguments[1]));
                            m.SendMessage("MaxDelay = {0}", XmlSpawner.defMaxDelay);
                        }
                        catch { m.SendMessage("invalid value : {0}", e.Arguments[1]); }
                    }
                    else
                                if (e.Arguments[0].ToLower(Core.Culture) == "mindelay")
                    {
                        try
                        {
                            XmlSpawner.defMinDelay = TimeSpan.FromMinutes(Convert.ToSingle(e.Arguments[1]));
                            m.SendMessage("MinDelay = {0}", XmlSpawner.defMinDelay);
                        }
                        catch { m.SendMessage("invalid value : {0}", e.Arguments[1]); }
                    }
                    else
                                    if (e.Arguments[0].ToLower(Core.Culture) == "spawnrange")
                    {
                        try
                        {
                            ...
                            m.SendMessage("SpawnRange = {0}", XmlSpawner.defSpawnRange);
                                        ...
                                        if (e.Arguments[0].ToLower(Core.Culture) == "homerange")
                    {
                        try
                        {
                            ...
                            m.SendMessage("HomeRange = {0}", XmlSpawner.defHomeRange);
                                            ...
                                            if (e.Arguments[0].ToLower(Core.Culture) == "relativehome")
                    {
                        try
                        {
                            ...
                            m.SendMessage("RelativeHome = {0}", XmlSpawner.defRelativeHome);
                                                ...
                                                if (e.Arguments[0].ToLower(Core.Culture) == "proximitytriggersound")
                    {
                        try
                        {
                            ...
                            m.SendMessage("ProximityTriggerSound = {0}", XmlSpawner.defProximityTriggerSound);
                                                    ...
                                                    if (e.Arguments[0].ToLower(Core.Culture) == "proximityrange")
                    {
                        try
                        {
                            ...
                            m.SendMessage("ProximityRange = {0}", XmlSpawner.defProximityRange);
                                                        ...
                                                        if (e.Arguments[0].ToLower(Core.Culture) == "triggerprobability")
                    {
                        try
                        {
                            XmlSpawner.defTriggerProbability = Convert.ToSingle(e.Arguments[1]);
                            m.SendMessage("TriggerProbability = {0}", XmlSpawner.defTriggerProbability);
                        }
                        catch { m.SendMessage("invalid value : {0}", e.Arguments[1]); }
                    }
                    else
                                                            if (e.Arguments[0].ToLower(Core.Culture) == "todstart")
                    {
                        try
                        {
                            XmlSpawner.defTODStart = TimeSpan.FromMinutes(Convert.ToSingle(e.Arguments[1]));
                            m.SendMessage("TODStart = {0}", XmlSpawner.defTODStart);
                        }
                        catch { m.SendMessage("invalid value : {0}", e.Arguments[1]); }
                    }
                    else
                                                                if (e.Arguments[0].ToLower(Core.Culture) == "todend")
                    {
                        try
                        {
                            XmlSpawner.defTODEnd = TimeSpan.FromMinutes(Convert.ToSingle(e.Arguments[1]));
                            m.SendMessage("TODEnd = {0}", XmlSpawner.defTODEnd);
                        }
                        catch { m.SendMessage("invalid value : {0}", e.Arguments[1]); }
                    }
                    else
                                                                    if (e.Arguments[0].ToLower(Core.Culture) == "stackamount")
                    {
                        try
                        {
                            ...
                            m.SendMessage("StackAmount = {0}", XmlSpawner.defAmount);
                                                                        ...
                                                                        if (e.Arguments[0].ToLower(Core.Culture) == "duration")
                    {
                        try
                        {
                            XmlSpawner.defDuration = TimeSpan.FromMinutes(Convert.ToSingle(e.Arguments[1]));
                            m.SendMessage("Duration = {0}", XmlSpawner.defDuration);
                        }
                        catch { m.SendMessage("invalid value : {0}", e.Arguments[1]); }
                    }
                    else
                                                                            if (e.Arguments[0].ToLower(Core.Culture) == "group")
                    {
                        try
                        {
                            ...
                            m.SendMessage("Group = {0}", XmlSpawner.defIsGroup);
                                                                                ...
                                                                                if (e.Arguments[0].ToLower(Core.Culture) == "team")
                    {
                        try
                        {
                            ...
                            m.SendMessage("Team = {0}", XmlSpawner.defTeam);
                                                                                    ...
                                                                                    if (e.Arguments[0].ToLower(Core.Culture) == "todmode")
                    {
                        ...
                        try
                        {
                            ...
                            m.SendMessage("TODMode = {0}", XmlSpawner.defTODMode);
                                                                                        ...
                                                                                        if (e.Arguments[0].ToLower(Core.Culture) == "maxrefractory")
                    {
                        try
                        {
                            XmlSpawner.defMaxRefractory = TimeSpan.FromMinutes(Convert.ToSingle(e.Arguments[1]));
                            m.SendMessage("MaxRefractory = {0}", XmlSpawner.defMaxRefractory);
                        }
                        catch { m.SendMessage("invalid value : {0}", e.Arguments[1]); }
                    }
                    else
                                                                                            if (e.Arguments[0].ToLower(Core.Culture) == "minrefractory")
                    {
                        try
                        {
                            XmlSpawner.defMinRefractory = TimeSpan.FromMinutes(Convert.ToSingle(e.Arguments[1]));
        ...
        private static void SpawnEditorGo_OnCommand(CommandEventArgs e)
        {
            ...
            if (from != null && e.Length >= 1)
            {
                ...
                else if (string.Compare(MapName, Map.DungeonSemiChiusi.Name, true) == 0)
                {
                    NewMap = Map.DungeonSemiChiusi;
                }
                else if (string.Compare(MapName, Map.DungeonChiusi.Name, true) == 0)
                {
                    NewMap = Map.DungeonChiusi;
                }
                else if (string.Compare(MapName, Map.Eventi.Name, true) == 0)
                {
                    NewMap = Map.Eventi;
                }
                else
                ...
                //rimossi riferimenti mappa malas e tokuno
        ...
        public static void SmartStat_OnCommand(CommandEventArgs e)
        {
            ...
            if (e.Arguments.Length > 1 && e.Arguments[0].ToLower(Core.Culture) == "accesslevel" && e.Mobile.AccessLevel >= AccessLevel.Administrator)
            {
                try
                {
                ...
                if (item is XmlSpawner)
                {
                    ...
                    for (int i = 0; i < nso; ++i)
                    {
                        ...
                        if (typestr == null || (type != null && (type == typeof(BaseVendor) || type.IsSubclassOf(typeof(BaseVendor)))) ||
                            ((type == null) && (!BaseXmlSpawner.IsTypeOrItemKeyword(typestr) && typestr.IndexOf('{') == -1 && !typestr.StartsWith("*", StringComparison.Ordinal) && !typestr.StartsWith("#", StringComparison.Ordinal))))
                        {
                            skipit = true;
                            break;
                        }
        ...
        public static void XmlUnLoadFromStream(Stream fs, string filename, string SpawnerPrefix, Mobile from, out int processedmaps, out int processedspawners)
        {
                ...
                if (ds.Tables[SpawnTablePointName] != null && ds.Tables[SpawnTablePointName].Rows.Count > 0)
                {
                    foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
                    {
                        ...
                        if (SpawnerPrefix == null || (SpawnerPrefix.Length == 0) || (SpawnName.StartsWith(SpawnerPrefix) == true))
                        {
                            ...
                            else if (string.Compare(XmlMapName, Map.DungeonSemiChiusi.Name, true) == 0 || XmlMapName == "Dungeon semi-chiusi")
                            {
                                SpawnMap = Map.DungeonSemiChiusi;
                                DungeonSemiChiusi++;
                            }
                            else if (string.Compare(XmlMapName, Map.DungeonChiusi.Name, true) == 0 || XmlMapName == "Dungeon chiusi")
                            {
                                SpawnMap = Map.DungeonChiusi;
                                DungeonChiusi++;
                            }
                            else if (string.Compare(XmlMapName, Map.Eventi.Name, true) == 0 || XmlMapName == "Eventi")
                            {
                                SpawnMap = Map.Eventi;
                                Eventi++;
                            }
            ...
            if (from != null)
            {
                from.SendMessage("{0}/{9} spawner(s) were unloaded using file {1} [Trammel={2}, Felucca={3}, Ilshenar={4}, Dungeon Semi-Chiusi={5}, Dungeon Chiusi={6}, Eventi={7}, Altri={8}].",
                    spawners_deleted, filename, TrammelCount, FeluccaCount, IlshenarCount, DungeonSemiChiusi, DungeonChiusi, Eventi, OtherCount, TotalCount);
            }
        ...
        public static void XmlImportMap(string filename, Mobile from, out int processedmaps, out int processedspawners)
        {
            ...
            if (System.IO.File.Exists(filename) == true)
            {
                ...
                int overridemap = -1;
                float overridemintime = -1;
                float overridemaxtime = -1;
        ...
        private static void ParseNewMapFormat(Mobile from, string filename, string line, string[] args, int linenumber, ref int spawnercount, ref int badspawnercount, ref int overridemap, ref float overridemintime, ref float overridemaxtime)
        {
            ...
            if (args.Length == 2 && (args[0].ToLower(Core.Culture) == "overridemap"))
            {
                try
                {
                ...
                if (args.Length == 2 && (args[0].ToLower(Core.Culture) == "overridemintime"))
            {
                try
                {
                    overridemintime = float.Parse(args[1]);
                }
                catch { }
            }
            else
                    if (args.Length == 2 && (args[0].ToLower(Core.Culture) == "overridemaxtime"))
            {
                try
                {
                    overridemaxtime = float.Parse(args[1]);
                }
                catch { }
            }
            else
                        ...
                        if (args.Length > 0 && (args[0] == "*"))
            {
                ...
                try
                {
                    ...
                    mindelay = float.Parse(args[11]);
                    maxdelay = float.Parse(args[12]);
                ...
                if (!badspawn && totalspawns > 0)
                {
                    ...
                    List<SpawnObject> so = new List<SpawnObject>(totalspawns);
                    int count = 0;
                    bool hasvendor = true;
                    for (int k = 0; k < 6; k++)
                    {
                    ...
                    if (map == 0)
                    {
                            ...
                            TimeSpan.FromHours(0), null, false, null)
                        {
                            ...
                            LastModifiedBy = fromname,
                            FirstModifiedBy = fromname
                        };
                        spawner.MoveToWorld(new Point3D(x, y, z), spawnmap);
                        if (spawner.Map == Map.Internal)
                        {
        ...
        private static void ParseOldMapFormat(Mobile from, string filename, string line, string[] args, int linenumber, ref int spawnercount, ref int badspawnercount, ref int overridemap, ref float overridemintime, ref float overridemaxtime)
        {
            // format of each .map line is * Dragon:Wyvern 5209 965 -40 2 2 10 50 30 1
            //  * typename:typename:... x y z map mindelay maxdelay homerange spawnrange maxcount
            // or
                        ...
                        if (args.Length > 0 && (args[0] == "*"))
            {
                ...
                else
                {
                    ...
                    if (args.Length == 11)
                    {
                        ...
                        try
                        {
                            ...
                            mindelay = float.Parse(args[6]);
                            maxdelay = float.Parse(args[7]);
                ...
                if (!badspawn && typenames != null && typenames.Length > 0)
                {
                    ...
                    // allow it to make an xmlspawner instead
                    // first add all of the creatures on the list
                    List<SpawnObject> so = new List<SpawnObject>(typenames.Length);
        ...
        public static void XmlImportSpawners_OnCommand(CommandEventArgs e)
        {
            if (e.Arguments.Length >= 1)
            {
                ...
                string filePath = Path.Combine(string.Format("{0}/Spawners", World.PrimaryName), filename);
                if (File.Exists(filePath))
                {
                    XmlDocument doc = new XmlDocument();
                    try
        ...
        private static void ImportSpawner(XmlElement node, Mobile from)
        {
            ...
            // allow it to make an xmlspawner instead
            // first add all of the creatures on the list
            List<SpawnObject> so = new List<SpawnObject>(creaturesName.Count);
        ...
        private static void ImportMegaSpawner(Mobile from, XmlElement node)
        {
            ...
            //bool.Parse(GetText(node["Active"], "True"));
            Point3D location = Point3D.Parse(GetText(node["Location"], "Error"));
            Map map = Map.Parse(GetText(node["Map"], "Error"));
            ...
            if (listnode != null)
            {
                ...
                if (nentries > 0)
                {
                    so = new List<SpawnObject>(nentries);
                        ...
                        if (entrynode != null)
                        {
                            ...
                            if (entrycount > nentries)
                            {
                                ...
                                try
                                {
                                    using (StreamWriter op = new StreamWriter("badimport.log", true))
                                    {
                                        op.WriteLine("{0} MSFImport Error; inconsistent entry count {1} {2}", Core.MistedDateTime, location, map);
                                        op.WriteLine();
                                    }
                                }
                                catch { }
                    ...
                    if (diff)
                    {
                        ...
                        try
                        {
                            using (StreamWriter op = new StreamWriter("badimport.log", true))
                            {
                                op.WriteLine("{0} MSFImport: Individual entry setting differences listed above from spawner at {1} {2}", Core.MistedDateTime, location, map);
                                op.WriteLine();
                            }
                        }
                        catch { }
        ...
        public static void XmlLoadFromStream(Stream fs, string filename, string SpawnerPrefix, Mobile from, Point3D fromloc, Map frommap, bool loadrelative, int maxrange, bool loadnew, out int processedmaps, out int processedspawners, bool verbose)
        {
            ...
            int FeluccaCount = 0;
            int IlshenarCount = 0;
            int DungSemiChiusiCount = 0;
            int DungChiusiCount = 0;
            int EventiCount = 0;
            int OtherCount = 0;
                ...
                if (ds.Tables[SpawnTablePointName] != null && ds.Tables[SpawnTablePointName].Rows.Count > 0)
                {
                    foreach (DataRow dr in ds.Tables[SpawnTablePointName].Rows)
                    {
                        ...
                        if ((SpawnerPrefix == null) || (SpawnerPrefix.Length == 0) || (SpawnName.StartsWith(SpawnerPrefix) == true))
                        {
                            ...
                            try { SpawnMinRefractory = TimeSpan.FromMinutes(float.Parse((string)dr["MinRefractory"])); }
                            catch { }

                            TimeSpan SpawnMaxRefractory = TimeSpan.FromMinutes(0);
                            try { SpawnMaxRefractory = TimeSpan.FromMinutes(float.Parse((string)dr["MaxRefractory"])); }
                            catch { }

                            TimeSpan SpawnTODStart = TimeSpan.FromMinutes(0);
                            try { SpawnTODStart = TimeSpan.FromMinutes(float.Parse((string)dr["TODStart"])); }
                            catch { }

                            TimeSpan SpawnTODEnd = TimeSpan.FromMinutes(0);
                            try { SpawnTODEnd = TimeSpan.FromMinutes(float.Parse((string)dr["TODEnd"])); }
                            catch { }
                            ...
                            float SpawnTriggerProbability = 1;
                            try { SpawnTriggerProbability = float.Parse((string)dr["TriggerProbability"]); }
                            catch { }
                            ...
                            try { SpawnDuration = TimeSpan.FromMinutes(float.Parse((string)dr["Duration"])); }
                            catch { }

                            TimeSpan SpawnDespawnTime = TimeSpan.FromHours(0);
                            try { SpawnDespawnTime = TimeSpan.FromHours(float.Parse((string)dr["DespawnTime"])); }
                            catch { }
                            ...
                            if (bad_spawner)
                            {
                                ...
                                try
                                {
                                    ...
                                    {
                                        op.WriteLine("# Invalid spawner : {0}: Fileposition {1} {2}", Core.MistedDateTime, fileposition, filename);
                                        op.WriteLine();
                                    }
                                }
                                catch { }
                                ...
                                if (questionable_spawner)
                            {
                                ...
                                try
                                {
                                    using (StreamWriter op = new StreamWriter("badxml.log", true))
                                    {
                                        op.WriteLine("# Questionable spawner : {0}: Format: X Y Z Map SpawnerName Fileposition Xmlfile", Core.MistedDateTime);
            ...
            if (from != null)
            {
                from.SendMessage("{0} spawner(s) were created from file {1} [Trammel={2}, Felucca={3}, Ilshenar={4}, Dungeon Semi-Chiusi={5}, Dungeon Chiusi={6} Eventi={7} Altri={8}].",
                    TotalCount, filename, TrammelCount, FeluccaCount, IlshenarCount, DungSemiChiusiCount, DungChiusiCount, EventiCount, OtherCount);
            }
        ...
        public static void Load_OnCommand(CommandEventArgs e)
        {
            if (e.Mobile.AccessLevel >= DiskAccessLevel)
            {
                if (e.Arguments.Length >= 1)
                {
                    ...
                    XmlLoadFromFile(filename, SpawnerPrefix, e.Mobile, false, 0, false, out _, out _);
                }
                else
                {
                    e.Mobile.SendMessage("Usage:  {0} <SpawnFile or directory> [SpawnerPrefixFilter]", e.Command);
                ...
                if (e.Arguments.Length >= 1)
                {
                    ...
                    try
                    {
                        ...
                        for (int nxtarg = 1; nxtarg < e.Arguments.Length; nxtarg++)
                        {
                            ...
                            if (e.Arguments[nxtarg].ToLower(Core.Culture) == "-maxrange")
                            {
        ...
        public class XmlSaveSingle : BaseCommand
        {
            ...
            public override void Execute(CommandEventArgs e, object obj)
            {
                ...
                {
                    e.Mobile.SendMessage("Puoi selezionare solo XmlSpawner, non è possibile salvare altri oggetti");
                    return;
                }

                Mobile m = e.Mobile;

                CommandLogging.WriteLine(m, "{0} {1} Salva l'XmlSpawner {2} su file {3}", m.AccessLevel, CommandLogging.Format(m), CommandLogging.Format(xmlspawner), CommandLogging.Format(filename));
        ...
        private static void SaveSpawns(Mobile m, XmlSpawner xmlspawner, string filename)
        {
            ...
            if (System.IO.Directory.Exists(XmlSpawner.XmlSpawnDir) && filename != null && !filename.StartsWith("/", StringComparison.Ordinal) && !filename.StartsWith("\\", StringComparison.Ordinal))
            {
            ...
            m.SendMessage("Salvo l'oggetto nella cartella {0} - file {1} - spawner {2}.", dirname, filename, xmlspawner);
        ...
        public static bool SaveSpawnList(Mobile from, List<XmlSpawner> savelist, string dirname, Stream stream, bool oldformat, bool verbose)
        {
            ...
            foreach (XmlSpawner sp in savelist)
            {
                ...
                else if (string.Compare(sp.Map.Name, Map.DungeonSemiChiusi.Name, true) == 0)
                {
                    DungeonSemiChiusi++;
                }
                else if (string.Compare(sp.Map.Name, Map.DungeonChiusi.Name, true) == 0)
                {
                    DungeonChiusi++;
                }
                else if (string.Compare(sp.Map.Name, Map.Eventi.Name, true) == 0)
                {
                    Eventi++;
                }
   ...
   TokunoCount++;*/
                ...
                dr["TODStart"] = sp.TODStart.TotalMinutes;
                dr["TODEnd"] = sp.TODEnd.TotalMinutes;
                dr["TODMode"] = (int)sp.TODMode;
                dr["KillReset"] = sp.KillReset;
                dr["MinRefractory"] = sp.RefractMin.TotalMinutes;
                dr["MaxRefractory"] = sp.RefractMax.TotalMinutes;
                dr["Duration"] = sp.m_Duration.TotalMinutes;
                dr["DespawnTime"] = sp.DespawnTime.TotalHours;
            ...
            if (from != null)
            {
                from.SendMessage("{0} spawner(s) were saved to file {1} [Trammel={2}, Felucca={3}, Ilshenar={4}, Dungeon Semi-Chiusi={5}, Dungeon Chiusi={6}, Eventi={7}, Altri={8}].",
                    TotalCount, dirname, TrammelCount, FeluccaCount, IlshenarCount, DungeonSemiChiusi, DungeonChiusi, Eventi, OtherCount);
            }
            return true;
...
#if (XTRACE)
        [Usage("XmlTrace")]
        [Description("Traces system load of XmlSpawners")]
        ...
        [Usage("XmlResetTrace")]
        [Description("Resets active tracing")]
            ...
            : base(BaseItemId)
        {
            ...
            InitSpawn(0, 0, m_Width, m_Height, string.Empty, 0, defMinDelay, defMaxDelay, defDuration,
                defProximityRange, defProximityTriggerSound, defAmount, defTeam, defHomeRange, defRelativeHome, new List<SpawnObject>(), defMinRefractory, defMaxRefractory,
            ...
            : base(BaseItemId)
        {
            ...
            List<SpawnObject> so = new List<SpawnObject>() { new SpawnObject(creatureName, amount) };
            ...
            : base(BaseItemId)
        {
            PlayerCreated = true;
            UniqueId = Guid.NewGuid().ToString();
            List<SpawnObject> so = new List<SpawnObject>() { new SpawnObject(creatureName, 1) };
            SpawnRange = defSpawnRange;
        ...
        public XmlSpawner(Guid uniqueId, int x, int y, int width, int height, string name, int maxCount, TimeSpan minDelay, TimeSpan maxDelay, TimeSpan duration,
            int proximityRange, int proximityTriggerSound, int amount, int team, int homeRange, bool isRelativeHomeRange, List<SpawnObject> spawnObjects,
            ...
            string itemTriggerName, string noitemTriggerName, string speechTrigger, string mobTriggerName, string mobPropertyName, string playerPropertyName, float triggerProbability,
        ...
        public void InitSpawn(int x, int y, int width, int height, string name, int maxCount, TimeSpan minDelay, TimeSpan maxDelay, TimeSpan duration,
            int proximityRange, int proximityTriggerSound, int amount, int team, int homeRange, bool isRelativeHomeRange, List<SpawnObject> objectsToSpawn,
            TimeSpan minRefractory, TimeSpan maxRefractory, TimeSpan todstart, TimeSpan todend, Item objectPropertyItem, string objectPropertyName, string proximityMessage,
            string itemTriggerName, string noitemTriggerName, string speechTrigger, string mobTriggerName, string mobPropertyName, string playerPropertyName, float triggerProbability,
            ...
            string configfile, TimeSpan despawnTime, string skillTrigger, bool smartSpawning, WayPoint wayPoint)
        {
            ...
            RefractMin = minRefractory;
            RefractMax = maxRefractory;
            TODStart = todstart;
            TODEnd = todend;
            TODMode = todMode;
            KillReset = killReset;
            ...
            // Assign the list of objects to spawn
            AddSpawnObjects(objectsToSpawn, true);//SpawnObjects = objectsToSpawn;
        ...
        public bool Spawn(bool smartspawn, byte loops)
        {
            if (m_SpawnObjects != null && m_SpawnObjects.Count > 0 && (m_proximityActivated || CanFreeSpawn) && TODInRange)
            {
                ...
                else
                {
                    ...
                    if (IndSpawn(SpawnIndex, smartspawn, loops))
                    {
                        if (!smartspawn)
                        {
        ...
        public bool IndSpawn(int index, bool smartspawn, int packrange, Point3D packcoord, bool ignoreloopprotection, byte loops)
        {
        ...
        // spawn an individual entry by index up to count times
        public bool IndSpawn(int index, bool smartspawn, byte loops)
        {
            return IndSpawn(index, smartspawn, false, loops);
        }

        // spawn an individual entry by index up to count times
        public bool IndSpawn(int index, bool smartspawn, bool ignoreloopprotection, byte loops)
        {
            return IndSpawn(index, smartspawn, -1, Point3D.Zero, ignoreloopprotection, loops);
        }
        ...
        public void Spawn(string SpawnObjectTypeName, bool smartspawn, int packrange, Point3D packcoord, byte loops)
        {
            ...
            for (int i = 0; i < m_SpawnObjects.Count; ++i)
            {
                if (m_SpawnObjects[i].TypeName.ToUpper(Core.Culture) == SpawnObjectTypeName.ToUpper(Core.Culture))
                {
            ...
            if (TheSpawn != null)
            {
                            ...
                            switch (keyvalueargs[0])
                            {
                                ...
                                case "#CONDITION":
                                    ...
                                    if (ckeyvalueargs.Length > 1)
                                    {
                                        ...
                                        if (!BaseXmlSpawner.CheckPropertyString(this, this, ckeyvalueargs[1], TriggerMob, out string error))
                                        {
                                            status_str = error;
                                            return false;
                                        }
                ...
                if (BaseXmlSpawner.IsTypeOrItemKeyword(typeName))
                {
                    ...
                    bool completedtypespawn = BaseXmlSpawner.SpawnTypeKeyword(this, TheSpawn, typeName, substitutedtypeName, requiresurface, spawnpositioning,
                        TriggerMob, Location, Map, SpawnerCB_Instance, out string status_str, loops);
                ...
                else
                {
                        ...
                        try
                        {
                            if (o is Mobile m)
                            {
								...
								*/
                                ...
                                if (!smartspawn || m is not BaseVendor)
                                {
                                    m.OnAfterSpawn();
                                    if(m.Deleted)
                                    {
                                ...
                                BaseXmlSpawner.ApplyObjectStringProperties(this, substitutedtypeName, m, TriggerMob, this, out string status_str);
                            ...
                            {
                                BaseXmlSpawner.AddSpawnItem(this, TheSpawn, item, Location, map, TriggerMob, requiresurface, spawnpositioning, substitutedtypeName, smartspawn, out string status_str);
                    ...
                    else
                    {
                        ...
                        {
                            DeleteEntry(TheSpawn, null, typeName);
                            InvalidateProperties();
                        }
                        else
                        {
            ...
            if (sgroup >= 0)
            {
                ...
                for (int j = 0; j < m_SpawnObjects.Count; j++)
                {
                    ...
                    if (so != null && so.SubGroup == sgroup)
                    {
                        ...
                        // get the SpawnsPerTick count and spawn up to that number
                        bool success = IndSpawn(j, smartspawn, so.PackRange, packcoord, ignoreloopprotection, loops);
                        m_TmpLocations = null;
        ...
        public void SortSpawns()
        {
            ...
            m_SpawnObjects.Sort(SubgroupSorter.Instance);
        }

        private class SubgroupSorter : IComparer<SpawnObject>
        {
            public static IComparer<SpawnObject> Instance = new SubgroupSorter();

            public SubgroupSorter()
            {
            }
        ...
        public void RefreshNextSpawnTime(SpawnObject so)
        {
            ...
            if (so.MinDelay < 0 || so.MaxDelay < 0)
            {
                so.NextSpawn = DateTime.UtcNow;
            }
            else
            {
                TimeSpan delay = TimeSpan.FromSeconds(Utility.RandomDouble((so.MinDelay * 60), (so.MaxDelay * 60)));
        ...
        private static WayPoint GetWaypoint(string waypointstr)
        {
            ...
            {
                string[] wayargs = BaseXmlSpawner.ParseString(waypointstr, 2, BaseXmlSpawner.CommaDelim);
        ...
        private bool CheckHoldSmartSpawning(object o)
        {
            ...
            if (!holdSmartSpawningHash.TryGetValue(o.GetType(), out PropertyInfo prop))
            {
        ...
        private void FindTileLocations(ref List<Point3D> locations, Map map, int startx, int starty, int width, int height, List<int> includetilelist, List<int> excludetilelist, TileFlag tileflag, bool checkitems, int spawnerZ)
        {
            ...
            for (int x = startx; x <= startx + width; ++x)
            {
                for (int y = starty; y <= starty + height; ++y)
                {
                    ...
                    if (checkitems)
                    {
                        IPooledEnumerable<Item> itemslist = map.GetItemsInRange(new Point3D(x, y, 0), 0);
        ...
        private List<Point3D> m_TmpLocations;
        public Point3D GetSpawnPosition(bool requiresurface, int packrange, Point3D packcoord, List<SpawnPositionInfo> spawnpositioning, Mobile mob)
        {
            ...
            //List<Point3D> m_TmpLocations = null;
                ...
                foreach (SpawnPositionInfo s in spawnpositioning)
                {
                    ...
                    switch (s.positionType)
                    {
                        ...
                        case SpawnPositionType.Tiles:
                        {
                            ...
                            {
                                if (!int.TryParse(positionargs[1], out start))
                                {
                                    start = -1;
                                }
                            }
                            ...
                            {
                                if (!int.TryParse(positionargs[2], out end))
                                {
                                    end = -1;
                                }
                            }
                        ...
                        case SpawnPositionType.Perimeter:
                            ...
                            if (positionargs != null && positionargs.Length > 1)
                            {
                                if (!int.TryParse(positionargs[1], out fillinc))
                                {
                                    fillinc = 1;
                                }
                            }
                        ...
                        case SpawnPositionType.Location:
                            ...
                            if (positionargs != null && positionargs.Length > 2)
                            {
                                int.TryParse(positionargs[1], out xinc);
                                int.TryParse(positionargs[2], out yinc);
                            }
                            if (positionargs != null && positionargs.Length > 3)
                            {
                                int.TryParse(positionargs[3], out zinc);
                            }
                            break;
                        case SpawnPositionType.Waypoint:
                            ...
                            if (positionargs != null && positionargs.Length > 2)
                            {
                                int.TryParse(positionargs[2], out positionrange);
                            }
                            ...
                            if (prefix != null)
                            {
                                ...
                                if (!spawnPositionWayTable.TryGetValue(prefix, out WayList) || WayList == null)
                                {
                                    ...
                                    List<Item> ilist = Map.GetAllItems();
                                    for (int i = ilist.Count - 1; i >= 0; --i)
                                    {
                                        if (ilist[i] is WayPoint && !string.IsNullOrEmpty(ilist[i].Name) && ilist[i].Name == prefix)
                                        {
                        ...
                        case SpawnPositionType.Player:
                            ...
                            if (positionargs != null && positionargs.Length > 1)
                            {
                                int.TryParse(positionargs[1], out positionrange);
                            }
                            break;
                    }
                }
            ...
            // precalculate tile locations if they have been specified - memorize them!
            if (m_TmpLocations == null && (includetilelist != null || excludetilelist != null || tileflag != TileFlag.None))
            {
                if (m_Region != null && HasRegionPoints(m_Region))
                {
                    FindRegionTileLocations(ref m_TmpLocations, m_Region, includetilelist, excludetilelist, tileflag, checkitems, Z);
                }
                else if (positioning == SpawnPositionType.Random)
                {
                    FindTileLocations(ref m_TmpLocations, Map, m_X, m_Y, m_Width, m_Height, includetilelist, excludetilelist, tileflag, checkitems, Z);
                }
            }
            ...
            for (int i = 0; i < 10; ++i)
            {
                    ...
                    if (m_Region != null && HasRegionPoints(m_Region))  // 2004.02.08 :: Omega Red
                {
                    ...
                    if (includetilelist != null || excludetilelist != null || tileflag != TileFlag.None)
                    {
                        ...
                        if (m_TmpLocations != null && m_TmpLocations.Count > 0)
                        {
                            Point3D p = m_TmpLocations[Utility.Random(m_TmpLocations.Count)];
                            x = p.X;
                            y = p.Y;
                            defaultZ = p.Z;
                        }
        ...
        private void DeleteFromList(List<IEntity> list)
        {
            if (list == null)
            {
                return;
        ...
        public void AddSpawnObject(string SpawnObjectName)
        {
            ...
            foreach (SpawnObject so in m_SpawnObjects)
            {
                if (so.TypeName.ToUpper(Core.Culture) == SpawnObjectName.ToUpper(Core.Culture))
                {
        ...
        public void DeleteSpawnObject(Mobile from, string SpawnObjectName)
        {
            ...
            try
            {
                ...
                if (TheSpawn != null)
                {
                    ...
                    if (delete_this_entry)
                    {
                        DeleteEntry(TheSpawn, from, SpawnObjectName);
                    }
                }
        ...
        public void DeleteEntry(XmlSpawner.SpawnObject toremove, Mobile from, string spawnobjectname)
        {
            if (m_SpawnObjects.Remove(toremove) && from != null)
            {
        ...
        public static object CreateObject(Type type, string itemtypestring, bool requireconstructable)
        {
            ...
            return CreateObject(type, typewordargs, requireconstructable, false, ConstructableAccessLevel);
        }
        ...
        public static object CreateObject(Type type, string itemtypestring, bool requireconstructable, bool requireattachable)
        {
            ...
            return CreateObject(type, typewordargs, requireconstructable, requireattachable, ConstructableAccessLevel);
        }

        public static object CreateObject(Type type, string[] typewordargs, bool requireconstructable, bool requireattachable, AccessLevel requester)
        {
            if (type == null)
            {
                return null;
...
#if (RESTRICTCONSTRUCTABLE)
                if ((!(requireconstructable && Add.IsConstructable(ctor, requester)) && !(requireattachable && XmlAttach.IsAttachable(ctor, requester))) && requester < AccessLevel.Developer)
                {
                    continue;
                }
#else
                        ...
                        if (m != null && (m.AccessLevel <= SmartSpawnAccessLevel || !m.Hidden))
                        {
                            ...
                            if (m.Map != null && m.Map != Map.Internal)
                            {
                                ...
                                if (s != null && GlobalSectorTable[m.Map.MapID] != null)
                                {
                                    ...
                                    if (GlobalSectorTable[m.Map.MapID].TryGetValue(s, out HashSet<XmlSpawner> spawnerlist) && spawnerlist != null)
                                    {
        ...
        private class WarnTimer2 : Timer
        {
            ...
            private class WarnEntry2
            {
                ...
                public ISpawnerItem m_Owner;

                public WarnEntry2(Point3D p, Map map, string name, ISpawnerItem owner)
                {
                    m_Point = p;
                    m_Map = map;
                    m_Name = name;
            ...
            public void Add(Point3D p, Map map, string name, ISpawnerItem owner)
            {
                m_List.Add(new WarnEntry2(p, map, name, owner));
            }
            ...
            protected override void OnTick()
            {
                try
                {
                    ...
                    using (StreamWriter op = new StreamWriter("badspawn.log", true))
                    {
                        op.WriteLine("# Bad spawns : {0}", Core.MistedDateTime);
                        op.WriteLine("# Format: X Y Z Map SpawnerName");
                        op.WriteLine();

                        foreach (WarnEntry2 e in m_List)
                        {
                            op.WriteLine("{0} {1} {2} {3}\t{4}\t({5})", e.m_Point.X, e.m_Point.Y, e.m_Point.Z, e.m_Map, e.m_Name, (e.m_Owner != null ? e.m_Owner.Serial.ToString() : "NULL"));
                        }
        ...
        public void DoTimer()
        {
            ...
            TimeSpan delay = TimeSpan.FromSeconds(Utility.RandomDouble(m_MinDelay.TotalSeconds, m_MaxDelay.TotalSeconds));
            DoTimer(delay);
        }
        ...
        public void DoTimer2(TimeSpan delay)
        {
            ...
            if (m_Duration > TimeSpan.Zero || (m_durActivated == true))
            {
                if (m_DurTimer != null)
                {
                    m_DurTimer.Stop();
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write(TODStart);
            writer.Write(TODEnd);
            writer.Write(RefractMin);
            writer.Write(RefractMax);
            if (m_refractActivated)
            {
                if (m_RefractoryTimer != null && m_RefractoryTimer.Running && m_RefractoryTimer.Delay > TimeSpan.Zero)
                {
                    writer.Write(m_RefractoryTimer.Delay);
                }
                else
                {
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            if (version <= 7 && version >= 0)
            {
                ...
                switch (version)
                {
                    ...
                    case 4:
                    {
                        ...
                        goto case 2;//this is the last for old spawner, 3 is for premium ones
                    }
                    case 3:
                    {
                        //this is for the premium spawner
                        m_HomeRange = reader.ReadInt();
                        reader.ReadInt();//spawnid
                        reader.ReadInt();//despawn
                        reader.ReadInt();//player range sensitive
                    ...
                    case 0:
                    {
                        ...
                        //m_SpawnObjects = new List<XmlSpawner.SpawnObject>(size);

                        for (int i = 0; i < size; ++i)
                        {
                            string creatureString = reader.ReadString();

                            m_SpawnObjects.Add(new SpawnObject(creatureString, m_Count));
                            string typeName = BaseXmlSpawner.ParseObjectType(creatureString);
                            if (ScriptCompiler.FindTypeByName(typeName) == null)
                            {
                                ...
                                m_WarnTimer.Add(Location, Map, typeName, this);
                            }
                        }

                        int count = reader.ReadInt();

                        List<ISpawnable> spawned = new List<ISpawnable>(count);
                        ...
                        for (int i = 0; i < count; ++i)
                        {
                            if (World.FindEntity(reader.ReadInt()) is ISpawnable e)
                            {
                                ...
                                foreach (SpawnObject so in m_SpawnObjects)
                                {
                                    if (so != null && so.TypeName != null && SpawnerType.GetType(so.TypeName) == e.GetType())
                                    {
            ...
            List<float> tmpSequentialResetTime = null;
            ...
            List<float> tmpMinDelay = null;
            List<float> tmpMaxDelay = null;
            ...
            switch (version)
            {
                ...
                case 26:
                {
                    ...
                    FirstModified = reader.ReadDateTime();
                ...
                case 24:
                {
                    ...
                    tmpMinDelay = new List<float>(tmpSpawnListSize);
                    tmpMaxDelay = new List<float>(tmpSpawnListSize);
                    ...
                    for (int i = 0; i < tmpSpawnListSize; ++i)
                    {
                        ...
                        if(version < 32)
                        {
                            mind = (float)reader.ReadDouble();
                            maxd = (float)reader.ReadDouble();
                        }
                        else
                        {
                            mind = reader.ReadFloat();
                            maxd = reader.ReadFloat();
                        }
                    ...
                    //we don't really want to reaload tons of shit
                    if (reader.ReadBool())
                    {
                ...
                case 21:
                {
                    DespawnTime = reader.ReadTimeSpan();
                    goto case 20;
                }
                case 20:
                {
                ...
                case 16:
                {
                    ...
                    tmpSequentialResetTime = new List<float>(tmpSpawnListSize);
                ...
                case 4:
                {
                    ...
                    TODStart = reader.ReadTimeSpan();
                    TODEnd = reader.ReadTimeSpan();
                    RefractMin = reader.ReadTimeSpan();
                    RefractMax = reader.ReadTimeSpan();
                ...
                case 0:
                {
                    ...
                    if (m_Width == m_Height && (m_Width % 2) == 0 && (m_X + (m_Width >> 1)) == X && (m_Y + (m_Height >> 1)) == Y)
                    {
                        m_SpawnRange = m_Width >> 1;
                    }
                    else
                    {
                        m_SpawnRange = -1;
                    ...
                    for (int i = 0; i < SpawnListSize; ++i)
                    {
                            ...
                            (!BaseXmlSpawner.IsTypeOrItemKeyword(typeName) && typeName.IndexOf('{') == -1 && !typeName.StartsWith("*") && !typeName.StartsWith("#"))))
                        {
                            if (string.IsNullOrEmpty(typeName))
                            {
                                Timer.DelayCall(() => DeleteEntry(TheSpawnObject, null, typeName));
                            }
                            else
                            {
                                if (m_WarnTimer == null)
        ...
        public class SpawnObject : ISpawnObjectFinder
        {
            ...
            private float m_MinDelay = -1;
            private float m_MaxDelay = -1;
            ...
            public int SubGroup { get => m_SubGroup; set => m_SubGroup = value; }
            public int SpawnsPerTick { get => m_SpawnsPerTick; set => m_SpawnsPerTick = value; }
            public int SequentialResetTo { get => m_SequentialResetTo; set => m_SequentialResetTo = value; }
            public int KillsNeeded { get => m_KillsNeeded; set => m_KillsNeeded = value; }
            public bool RestrictKillsToSubgroup { get => m_RestrictKillsToSubgroup; set => m_RestrictKillsToSubgroup = value; }
            public bool ClearOnAdvance { get => m_ClearOnAdvance; set => m_ClearOnAdvance = value; }
            public float MinDelay { get => m_MinDelay; set => m_MinDelay = value; }
            public float MaxDelay { get => m_MaxDelay; set => m_MaxDelay = value; }
            public bool Disabled { get => m_Disabled; set => m_Disabled = value; }
            public bool Ignore { get => m_Ignore; set => m_Ignore = value; }
            public int PackRange { get => m_PackRange; set => m_PackRange = value; }

            public override int GetHashCode()
            {
                return TypeName.ToUpper(Core.Culture).GetHashCode();
            }
            ...
            public bool Equals(SpawnObject so)
            {
                return so != null && so.TypeName.ToUpper(Core.Culture) == TypeName.ToUpper(Core.Culture);
            }
            ...
            public SpawnObject(string name, int maxamount, int subgroup, float sequentialresettime, int sequentialresetto, int killsneeded,
                bool restrictkills, bool clearadvance, float mindelay, float maxdelay, int spawnsper, int packrange)
            {
                TypeName = name;
                MaxCount = maxamount;
                SubGroup = subgroup;
            ...
            internal static List<SpawnObject> LoadSpawnObjectsFromString(string ObjectList)
            {
                ...
                return NewSpawnObjects;
            }



            internal static List<SpawnObject> LoadSpawnObjectsFromString2(string ObjectList)
            {
                // Clear the spawn object list
                List<XmlSpawner.SpawnObject> NewSpawnObjects = new List<XmlSpawner.SpawnObject>();
                        ...
                        if (SpawnObjectDetails.Length == 2)
                        {
                            ...
                            if (SpawnObjectDetails[0].Length > 0)
                            {
                                ...
                                if (SpawnObjectDetails[1].Length > 0)
                                {
                                    ...
                                    float resetTime = 0;
                                    try { resetTime = float.Parse(parmstr); }
                                    catch { }
                                    ...
                                    float minD = -1;
                                    try { minD = float.Parse(parmstr); }
                                    catch { }

                                    // MaxDelay
                                    parmstr = GetParm(s, ":DX=");
                                    float maxD = -1;
                                    try { maxD = float.Parse(parmstr); }
                                    catch { }
