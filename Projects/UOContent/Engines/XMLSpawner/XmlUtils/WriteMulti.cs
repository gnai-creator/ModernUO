...
namespace Server.Engines.XmlSpawner2
{
    public class WriteMulti
    {
        ...
        public static void Initialize()
        {
            ...
            CommandSystem.Register("WriteMulti", AccessLevel.GameMaster, new CommandEventHandler(WriteMulti_OnCommand));
        }
        ...
        public static void WriteMulti_OnCommand(CommandEventArgs e)
        {
            ...
            if (e.Mobile.AccessLevel < AccessLevel.GameMaster)//XmlSpawner.DiskAccessLevel)
            {
            ...
            bool includemultis, bool includeinvisible, bool includeaddons)
        {
            ...
            BoundingBoxPicker.Begin(m, (map, start, end) => DefineMultiArea_Callback(m, map, start, end, dirname, zmin, zmax, includeitems, includestatics, includemultis, includeinvisible, includeaddons));
        }

        private static void DefineMultiArea_Callback(Mobile from, Map map, Point3D start, Point3D end, string dirname, int zmin, int zmax, bool includeitems, bool includestatics, bool includemultis, bool includeinvisible, bool includeaddons)
        {
            if (from != null && map != null)
            {
                List<Item> itemlist = new List<Item>();
                List<TileEntry> staticlist = new List<TileEntry>();
                ...
                if (includeitems)
                {
                    ...
                    IPooledEnumerable<Item> eable = map.GetItemsInBounds(new Rectangle2D(sx, sy, ex - sx + 1, ey - sy + 1));
                    ...
                    for (int x = sx; x < ex; ++x)
                    {
                        for (int y = sy; y < ey; ++y)
                        {
                            ...
                            for (int j = 0; j < statics.Length; j++)
                            {
                                ...
                                {
                                    staticlist.Add(new TileEntry(statics[j].ID & TileData.MaxItemValue, x, y, statics[j].Z));
                                }
                            }
                        }
                    }
