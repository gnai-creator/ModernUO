...
namespace Server.Gumps
{
    public class XmlPartialCategorizedAddGump : Gump
    {
        ...
        public XmlPartialCategorizedAddGump(Mobile from, string searchString, int page, List<SearchEntry> searchResults, bool explicitSearch, int entryindex, Gump gump) : base(50, 50)
        {
        ...
        private static void Match(string match, Type[] types, List<SearchEntry> results)
        {
            if (match.Length == 0)
            {
                return;
            ...
            match = match.ToLower(Core.Culture);

            for (int i = 0; i < types.Length; ++i)
            {
                Type t = types[i];

                if ((typeofMobile.IsAssignableFrom(t) || typeofItem.IsAssignableFrom(t)) && t.Name.ToLower(Core.Culture).IndexOf(match) >= 0 && !results.Exists(se => se.EntryType == t))
                {
