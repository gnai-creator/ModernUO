    ...
    public class XmlEditDialogGump : Gump
    {
        ...
        private class EditDialogTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (!(XmlAttach.FindAttachment(ie, typeof(XmlDialog)) is XmlDialog xd))
                {
                    from.SendMessage("Target has no XmlDialog attachment");

                    from.SendGump(new XmlConfirmAddGump(from, ie));
        ...
        {
            private Mobile m_From;
            private IEntity m_Targeted;

            public XmlConfirmAddGump(Mobile from, IEntity targeted) : base(0, 0)
            {
                    ...
                    /*try
{
    m_Dialog.ProximityRange = int.Parse(tr.Text);
                ...
                if (tr != null)
                {
                    if (TimeSpan.TryParse(tr.Text, out TimeSpan ts))
                    {
            ...
            switch (info.ButtonID)
            {
                ...
                default:
                {
                        ...
                        if (info.ButtonID >= 5000 && info.ButtonID < 5100)
                    {
                        ...
                        XmlTextEntryBook book = new XmlTextEntryBook(1, string.Empty, m_Dialog.Name, 20, true, new XmlTextEntryBookCallback(ProcessXmlEditBookEntry), args);
                        ...
                        book.Visible = false;
                        book.Movable = false;
                        book.MoveToWorld(new Point3D(state.Mobile.Location.X, state.Mobile.Location.Y, state.Mobile.Location.Z - 50), state.Mobile.Map);
        ...
        public class XmlConfirmDeleteGump : Gump
        {
            ...
            private XmlEditDialogGump m_Gump;
