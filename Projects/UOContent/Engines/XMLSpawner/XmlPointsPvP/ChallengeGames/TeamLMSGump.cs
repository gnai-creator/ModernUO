...
namespace Server.Gumps
{
	public class TeamLMSGump : Gump
	{
		...
		private List<BaseChallengeEntry> m_WorkingList;
		...
		public TeamLMSGump( TeamLMSGauntlet gauntlet, Mobile from, int page ) : base( 20, 30 )
		{
			...
			if(gauntlet.Participants != null)
			{
                ...
                for(int i = 0;i<m_WorkingList.Count;++i)
                {
                    ...
                    BaseChallengeEntry entry = m_WorkingList[i];
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( info.ButtonID )
			{
                ...
                case 300:
                    ...
                    bool complete = true;
                    foreach(var entry in m_WorkingList)
                    {
                        if(entry != null)
                        {
                            Mobile m = entry.Participant;
                    ...
                    if(complete)
                    {
                        ...
                        foreach(BaseChallengeEntry entry in m_WorkingList)
                        {
                            if(entry != null)
                            {
                                Mobile m = entry.Participant;
				...
				default:
				{
				    ...
				    if(info.ButtonID >= 4000)
                    {
                        ...
                        {
                            var entry = m_WorkingList[selection];

                            // find the master participants list entry with the same participant
                            if(m_gauntlet.Participants != null)
                            {
                                BaseChallengeEntry forfeitentry = null;

                                foreach(var masterentry in m_gauntlet.Participants)
                                {
				    ...
				    if(info.ButtonID >= 2000)
                    {
                        ...
                        if(selection < m_WorkingList.Count)
                        {
                            ...
                            if(m_gauntlet.Participants != null)
                            {
                                BaseChallengeEntry kickentry = null;
		...
		public static void RefreshAllGumps( TeamLMSGauntlet gauntlet, bool force)
		{
            if(gauntlet.Participants != null)
            {
                foreach(var entry in gauntlet.Participants)
                {
        ...
        {
            private List<BaseChallengeEntry> m_list;
            private TeamLMSGauntlet m_gauntlet;

            public MemberTarget(TeamLMSGauntlet gauntlet, List<BaseChallengeEntry> list) :  base ( 30, false, TargetFlags.None )
            {
                m_list = list;
                m_gauntlet = gauntlet;
            }
            ...
            {
                if (from != null && !from.Deleted && m_gauntlet != null && m_list != null && targeted is PlayerMobile pm)
                {
                    ...
                    if(m_gauntlet.Participants != null)
                    {
                        foreach(var entry in m_gauntlet.Participants)
                        {
