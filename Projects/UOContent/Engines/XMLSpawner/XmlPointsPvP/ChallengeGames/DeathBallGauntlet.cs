...
namespace Server.Items
{
    public class DeathBall : Item
    {
		...
		public override bool CheckLift(Mobile from, Item item, ref LRReason reject)
        {
            ...
            if (XmlAttach.FindAttachment(from, typeof(XmlPoints)) is XmlPoints afrom && afrom.ChallengeGame != null && ((afrom.ChallengeGame is DeathBallGauntlet && (((DeathBallGauntlet)(afrom.ChallengeGame)).Ball == item))
    ...
    public class DeathBallGauntlet : BaseChallengeGame
    {
        ...
        public DeathBall Ball { get; private set; }
        ...
        {
            if(Ball != null && Ball.RootParent is Mobile owner)
            {
                // only score if one player is carrying the ball
                BaseChallengeEntry entry = GetParticipant(owner);
            ...
            else
            {
                ...
                if(Participants != null)
    		    {
                    ...
                    {
                        if(entry.Status == ChallengeStatus.Active && entry.Participant != null && entry.Participant.Holding == Ball)
                        {
                            // bump their score
                            entry.Score++;
        ...
        public override void OnPlayerKilled(PlayerMobile killer, PlayerMobile killed)
        {
            ...
            if(Ball != null && Ball.RootParent is Corpse)
            {
                Mobile owner = ((Corpse)(Ball.RootParent)).Owner;
                if(owner == killed)
                {
                    GameBroadcast(100412, killed.Name);  // "{0} has dropped the ball!"
                    Ball.MoveToWorld(owner.Location, owner.Map);
                }
            }
        }
        ...
        {
            // drop the ball if they were carrying it
            if(Ball != null && from != null && Ball.RootParent == from)
            {
		...
		public Mobile BallOwner
		{
			...
			{
				Mobile owner = null;
				if (Ball != null && Ball.RootParent is Mobile)
				{
        ...
        public DeathBallGauntlet(PlayerMobile challenger) : base( 0x1414 )
        {
            ...
            {
                Name = $"DeathBall Gauntlet - Challenge by {challenger.Name}";
            }
        }
        ...
        public override void Deserialize( GenericReader reader )
        {
            ...
            case 0:
                ...
                Ball = (DeathBall)reader.ReadItem();
