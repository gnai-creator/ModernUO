...
namespace Server.Engines.XmlSpawner2
{
    ...
    public class ChallengeRegionStone : Item
    {
        ...
        public Rectangle3D[] ChallengeArea { get; set; }
        ...
        public bool DisableGuards
        {
            ...
            set
            {
                ...
                {
                    ChallengeRegion.Unregister();
                    ChallengeRegion.Disabled = m_Disable;
                    ChallengeRegion.Register();
                }
            }
        }
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public ChallengeGameRegion ChallengeRegion { get; private set; }
        ...
        public void RefreshRegions()
        {
            ...
            if (m_ChallengeMap == null || ChallengeArea == null) return;

            // define a new one based on the named region
            ChallengeRegion = new ChallengeGameRegion(m_ChallengeRegionName, m_ChallengeMap, m_Priority, ChallengeArea);
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            // do the coord list
            if (ChallengeRegion != null && ChallengeRegion.Area != null && ChallengeRegion.Area.Length > 0)
            {
                writer.Write(ChallengeRegion.Area.Length);

                for (int i = 0; i < ChallengeRegion.Area.Length; ++i)
                {
                    writer.Write(ChallengeRegion.Area[i].Start);
                    writer.Write(ChallengeRegion.Area[i].End);
                }
            }
            else
            {
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            ChallengeArea = new Rectangle3D[0];
                ...
                case 0:
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        if (version < 2)
                        {
                            ...
                            ChallengeArea = Region.ConvertTo3D(area);
                        }
                        else
                        {
                            ChallengeArea = new Rectangle3D[count];
                            for (int i = 0; i < count; ++i)
                            {
                                ChallengeArea[i] = new Rectangle3D(reader.ReadPoint3D(), reader.ReadPoint3D());
                            }
                        }
                    }
                    break;
        ...
        {
            BoundingBoxPicker.Begin(m, (map, start, end) => ChallengeRegionArea_Callback(m, map, start, end, this));
        }

        private static void ChallengeRegionArea_Callback(Mobile from, Map map, Point3D start, Point3D end, ChallengeRegionStone s)
        {
            // assign these coords to the region
            s.ChallengeArea = new Rectangle3D[1];
            s.ChallengeArea[0] = Region.ConvertTo3D(new Rectangle2D(start.X, start.Y, end.X - start.X + 1, end.Y - start.Y + 1));
