...
namespace Server.Items
{
    ...
    public class CombinationLock : Item
    {
        ...
        public int CheckDigit(object o, string property)
        {
            ...
            string[] argtest = BaseXmlSpawner.ParseString(property, 2, BaseXmlSpawner.ComparerDelim);
            if (argtest.Length > 1)
            {
                // ok, its a test, so test it
                if (BaseXmlSpawner.CheckPropertyString(null, o, property, null, out _))
                {
                    return 1; // true
                }
                else
            ...
            string[] arglist = BaseXmlSpawner.ParseString(result, 2, BaseXmlSpawner.EqualDelim);
            if (arglist.Length < 2)
            {
                return -1;
            }

            string[] arglist2 = BaseXmlSpawner.ParseString(arglist[1], 2, BaseXmlSpawner.SpaceDelim);
