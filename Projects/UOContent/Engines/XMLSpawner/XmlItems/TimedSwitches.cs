...
namespace Server.Items
{
    ...
    public class TimedLever : XmlLatch, ILinkable
    {
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public bool DisTimedOnUse { get; set; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int DisTimedSeconds { get; set; }
        ...
        private ISpawnerItem m_SpawnerLink;

        [CommandProperty(AccessLevel.GameMaster)]
        public Item SpawnerLink { get => m_SpawnerLink as Item; set => m_SpawnerLink = value as ISpawnerItem; }
        ...
        public void Activate(Mobile from, int state, List<ILinkable> links)
        {
            ...
            if (from != null)
            {
                if (m_SpawnerLink != null && !m_SpawnerLink.isEmpty())
                {
                    //if(from!=null)
                    from.SendLocalizedMessage(505342);// "La leva sembra essere bloccata!");
                    return;
                }
                if (DisTimedOnUse /*&& from!=null*/)
                {
                    if (m_NextActivation > DateTime.UtcNow && links == null)
                    {
                        from.SendLocalizedMessage(505342);// "La leva sembra essere bloccata!");
                        return;
                    }
                    ...
                    {
                        m_NextActivation = DateTime.UtcNow.Add(TimeSpan.FromSeconds(DisTimedSeconds));
                    }
                }
            }
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            // version 3
            writer.Write(DisTimedOnUse);
            writer.Write(DisTimedSeconds);
        ...
        public override void Deserialize(GenericReader reader)
        {
            ...
            switch (version)
            {
                case 4:
                {
                    m_SpawnerLink = reader.ReadItem() as ISpawnerItem;
                    goto case 3;
                }
                case 3:
                {
                    DisTimedOnUse = reader.ReadBool();
                    DisTimedSeconds = reader.ReadInt();
