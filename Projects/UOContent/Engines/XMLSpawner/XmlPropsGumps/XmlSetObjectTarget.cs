...
namespace Server.Gumps
{
    public class XmlSetObjectTarget : Target
    {
        ...
        public XmlSetObjectTarget(PropertyInfo prop, Mobile mobile, object o, Stack<PropertiesGump.StackEntry> stack, Type type, int page, List<PropertyObject> list) : base(-1, false, TargetFlags.None)
        {
            m_Property = prop;
            m_Mobile = mobile;
            m_Object = o;
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            try
            {
                ...
                {
                    CommandLogging.LogChangeProperty(m_Mobile, m_Object, m_Property.Name, string.Format("{0} -> {1}", m_Property.GetValue(m_Object) ?? "(-null-)", targeted ?? "(-null-)"));//targeted.ToString() );
