...
namespace Server.Gumps
{
    public class XmlSetObjectGump : Gump
    {
        ...
        private static int TotalHeight = OffsetSize + (5 * (EntryHeight + OffsetSize));
        ...
        public XmlSetObjectGump(PropertyInfo prop, Mobile mobile, object o, Stack<PropertiesGump.StackEntry> stack, Type type, int page, List<PropertyObject> list) : base(GumpOffsetX, GumpOffsetY)
        {
            m_Property = prop;
            m_Mobile = mobile;
            m_Object = o;
        ...
        private class InternalPrompt : Prompt
        {
            ...
            public InternalPrompt(PropertyInfo prop, Mobile mobile, object o, Stack<PropertiesGump.StackEntry> stack, Type type, int page, List<PropertyObject> list)
            {
                m_Property = prop;
                m_Mobile = mobile;
                m_Object = o;
            ...
            public override void OnResponse(Mobile from, string text)
            {
                ...
                if (shouldSet)
                {
                    try
                    {
                        CommandLogging.LogChangeProperty(m_Mobile, m_Object, m_Property.Name, string.Format("{0} -> {1}", m_Property.GetValue(m_Object) ?? "(-null-)", toSet ?? "(-null-)"));
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (shouldSet)
            {
                try
                {
                    CommandLogging.LogChangeProperty(m_Mobile, m_Object, m_Property.Name, string.Format("{0} -> {1}", m_Property.GetValue(m_Object) ?? "(-null-)", toSet ?? "(-null-)"));//toSet==null?"(null)":toSet.ToString() );
                    m_Property.SetValue(m_Object, toSet, null);
                }
                catch
                {
