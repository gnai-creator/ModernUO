...
namespace Server.Gumps
{
    public class XmlSetCustomEnumGump : XmlSetListOptionGump
    {
        ...
        public XmlSetCustomEnumGump(PropertyInfo prop, Mobile mobile, object o, Stack<PropertiesGump.StackEntry> stack, int propspage, List<PropertyObject> list, string[] names) : base(prop, mobile, o, stack, propspage, list, names, null)
        {
            m_Names = names;
        }
        ...
        public override void OnResponse(NetState sender, RelayInfo relayInfo)
        {
            ...
            if (index >= 0 && index < m_Names.Length)
            {
                try
                {
                    ...
                    CommandLogging.LogChangeProperty(m_Mobile, m_Object, m_Property.Name, string.Format("{0} -> {1}", m_Property.GetValue(m_Object), m_Names[index]));
