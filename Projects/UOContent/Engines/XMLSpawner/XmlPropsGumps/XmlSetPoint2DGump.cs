...
namespace Server.Gumps
{
    public class XmlSetPoint2DGump : Gump
    {
        ...
        private static int CoordWidth = 105;
        private static int EntryWidth = CoordWidth + OffsetSize + CoordWidth;
        ...
        public XmlSetPoint2DGump(PropertyInfo prop, Mobile mobile, object o, Stack<PropertiesGump.StackEntry> stack, int page, List<PropertyObject> list) : base(GumpOffsetX, GumpOffsetY)
        {
            m_Property = prop;
            m_Mobile = mobile;
            m_Object = o;
        ...
        private class InternalTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                if (targeted is IPoint3D p)
                {
                    try
                    {
                        CommandLogging.LogChangeProperty(m_Mobile, m_Object, m_Property.Name, string.Format("{0} -> {1}", m_Property.GetValue(m_Object), new Point2D(p)));//new Point2D( p ).ToString() );
