...
namespace Server.Mobiles
{
    public class TalkingBaseEscortable : TalkingBaseCreature
    {
        ...
        private static string[] m_TownNames = new string[]
            {
                "Cove", "Britain", "Stirling",
                "Minoc", "Ocllo", "Trinsic",
                "Vesper", "Dregoth", "Skara Brae",
        ...
        public virtual bool SayDestinationTo(Mobile m)
        {
            ...
            if (escorter == null)
            {
                Say("Stò cercando qualcuno che mi accompagni a {0}, lo faresti tu?", (dest.Name == "Ocllo" && m.Map == Map.Trammel) ? "Haven" : dest.Name);
                return true;
            }
            else if (escorter == m)
            {
                Say("Vai avanti allora! Ti pagherò non appena saremo arrivati in {0}.", (dest.Name == "Ocllo" && m.Map == Map.Trammel) ? "Haven" : dest.Name);
                return true;
            }
        ...
        private static TimeSpan m_EscortDelay = TimeSpan.FromHours(24.0);
        ...
        public virtual bool AcceptEscorter(Mobile m)
        {
            ...
            {
                Say("Hai già qualcuno che stai scortando.");
                return false;
            }
            else if (m is PlayerMobile && (((PlayerMobile)m).LastEscortTime + m_EscortDelay) >= DateTime.UtcNow)
            {
                int minutes = (int)Math.Ceiling(((((PlayerMobile)m).LastEscortTime + m_EscortDelay) - DateTime.UtcNow).TotalHours);

                Say("Dovresti riposare almeno {0} or{1} dall'ultimo viaggio, prima di cominciarne un altro.", minutes, minutes == 1 ? "a" : "e");
                return false;
            }
            else if (SetControlMaster(m))
            {
                ...
                Say("Vai avanti! Ti pagherò non appena saremo arrivati a {0}.", dest.Name);
        ...
        public virtual bool CheckAtDestination()
        {
            ...
            if (dest.Contains(Location, Map))
            {
                ...
                {
                    item = new Gold(11000, 22000);
                }
                else
                {
                    item = new Token() { Amount = Utility.RandomMinMax(1, 2) };
                }
                ...
                Misc.Titles.AwardFame(escorter, 100, true);
        ...
        public virtual string PickRandomDestination()
        {
            ...
            while (picked == null)
            {
                ...
                if (test != null && test.Contains(Location, Map))
                {
                    picked = null;
                }
            }
