...
namespace Server.Mobiles
{
    ...
    public delegate void AfterParserCallback(XmlSpawner spawner, Mobile trigmob, object refobject);
    ...
    public class BaseXmlSpawner
    {
        ...
        public static bool IsProtected(Type type, string property, PropertyInfo pinfo)
        {
            ...
            CPA attr = Properties.GetCPA(pinfo);
            if (attr != null && XmlSpawner.DiskAccessLevel < attr.WriteLevel)
            {
                return true;
            }
            ...
            {
                if ((p.ObjectType == type || type.IsSubclassOf(p.ObjectType)) && (property.ToLower(Core.Culture) == p.Name.ToLower(Core.Culture)))
                {
                    return true;
                }
            }
        ...
        public static Dictionary<Type, Dictionary<string, PropertyInfo>> TypeInfoCache { get; } = new Dictionary<Type, Dictionary<string, PropertyInfo>>();
        ...
        internal static char[] DotDelim { get; } = new char[1] { '.' };
        internal static char[] SpaceDelim { get; } = new char[1] { ' ' };
        internal static char[] SemicolonDelim { get; } = new char[1] { ';' };
        internal static char[] EqualDelim { get; } = new char[1] { '=' };
        internal static char[] SquareLeftDelim { get; } = new char[1] { '[' };
        internal static char[] AndOrDelim { get; } = new char[2] { '&', '|' };
        internal static char[] ComparerDelim { get; } = new char[4] { '=', '>', '<', '!' };
        internal static char[] OpenBracketDelim { get; } = new char[1] { '<' };
        private static char[] literalend { get; } = new char[1] { '§' };
        ...
        public static void RemoveKeyword(string name)
        {
            ...
            name = name.Trim().ToUpper(Core.Culture);
        ...
        public static void Configure()
        {
            ...
            AddTypemodKeyword("EMOTE");
            AddTypemodKeyword("SETVALUE");
            AddTypemodKeyword("SETVAR");
            ...
            AddValueKeyword("GETONSPAWNENTRY");
            ...
            AddValuemodKeyword("GETONSPAWNENTRY");
            ...
            private class KeywordTimer : Timer
            {
                ...
                protected override void OnTick()
                {
                    ...
                    if (m_Condition != null && m_Condition.Length > 0 && m_Spawner != null && m_Spawner.Running)
                    {
                        ...
                        if (TestItemProperty(m_Spawner, m_Spawner, m_Condition, trigmob, out _))
                        {
        ...
        public static string ConstructFromString(PropertyInfo p, Type type, object obj, string value, ref object constructed)
        {
            ...
            {
                if (ulong.TryParse(value.Substring(2), NumberStyles.HexNumber, Core.Culture, out ulong res))
                {
                    toSet = Convert.ChangeType(res, type);
                }
                else
                {
            ...
            else if (value.StartsWith("0x", StringComparison.Ordinal) && (IsItem(type) || IsMobile(type)))
            {
                if (int.TryParse(value.TrimEnd().Substring(2), NumberStyles.HexNumber, Core.Culture, out int ser))
                {
                    toSet = World.FindEntity(ser);
                    if (toSet == null)
                    {
                        return "Item or Mobile dont exist. cannot use.";
                    }
                }
                else
                {
        ...
        public static string InternalSetValue(Mobile from, object o, PropertyInfo p, string value, bool shouldLog, int index)
        {
            ...
            try
            {
                if (shouldLog)
                {
                    CommandLogging.LogChangeProperty(from, o, p.Name, string.Format("{0} -> {1}", p.GetValue(o), value));
                }
        ...
        public static string SetPropertyValue(XmlSpawner spawner, object o, string name, string value)
        {
            ...
            string[] arglist = ParseString(name, 2, DotDelim);

            string propname = arglist[0];

            string[] keywordargs = ParseString(propname, 4, CommaDelim);
            ...
            if (keywordargs[0] == "ATTACHMENT")
            {
                ...
                List<XmlAttachment> attachments = XmlAttach.FindAttachments(o as IEntity, attachtype, aname);
            ...
            else if (keywordargs[0] == "SKILL")
            {
                ...
                if (Enum.TryParse(keywordargs[1], true, out SkillName skillname))
                {
                    if (o is Mobile)
                    {
                        Skill skill = ((Mobile)o).Skills[skillname];
                        if (float.TryParse(value, NumberStyles.Float, Core.Culture, out float d))
                        {
                            skill.Base = d;
                            return "Property has been set.";
                        }
            ...
            if (arglist.Length == 2)
            {
                PropertyInfo plookup = LookupPropertyInfo(type, propname);
                if (plookup != null)
                {
                    //if ( !plookup.CanWrite )
                    //return "Property is read only.";

                    if (IsProtected(type, propname, plookup))
                    {
                        return "Property is protected.";
                    }

                    //plookup.PropertyType;
        ...
        public static string SetPropertyObject(XmlSpawner spawner, object o, string name, object value)
        {
            ...
            if (arglist.Length == 2)
            {
                ...
                PropertyInfo plookup = LookupPropertyInfo(type, arglist[0]);
                if (plookup != null)
                {
                    ...
                    if (IsProtected(type, arglist[0], plookup))
                    {
                        return "Property is protected.";
                    }
            ...
            else
            {
                ...
                // use the lookup table for optimization if possible
                PropertyInfo plookup = LookupPropertyInfo(type, name);
                if (plookup != null)
                {
                    if (!plookup.CanWrite)
                    {
                    ...
                    if (IsProtected(type, name, plookup))
                    {
                        return "Property is protected.";
                    }
        ...
        public static string GetBasicPropertyValue(object o, string propname, out Type ptype)
        {
            ...
            PropertyInfo pinfo = LookupPropertyInfo(type, propname);
            if (pinfo != null && pinfo.CanRead)
            {
                ptype = pinfo.PropertyType;
                string value = InternalGetValue(o, pinfo, -1);
        ...
        public static string GetPropertyValue(XmlSpawner spawner, object o, string name, out Type ptype)
        {
            ...
            else if (keywordargs[0] == "SKILL")
            {
                ...
                if (Enum.TryParse(keywordargs[1], true, out SkillName skillname))
                {
                    if (o is Mobile mob)
                    {
                        Skill skill = mob.Skills[skillname];
            ...
            else if (keywordargs[0] == "SERIAL")
            {
                if (o is IEntity ie)
                {
                    ptype = ie.Serial.GetType();

                    return string.Format("Serial = {0}", ie.Serial);
                }
                else
                {
        ...
        public static string ApplyToProperty(XmlSpawner spawner, object getobject, object setobject, string getpropertystring, string setpropertystring)
        {
            ...
            string getvalue = GetPropertyValue(spawner, getobject, getpropertystring, out Type ptype);
        ...
        public static bool ApplyObjectStringProperties(XmlSpawner spawner, string str, object o, Mobile trigmob, object refobject, out string status_str)
        {
            ...
            if (string.IsNullOrEmpty(str) || o == null)
            {
                return false;
            }
            ...
            while (arglist.Length > 1)
            {
                ...
                if (arglist.Length > 1)
                {
                    ...
                    string[] groupedarglist = ParseString(arglist[1], 2, SquareLeftDelim);

                    // take that argument list that should look like arg2/ag3/arg4>/arg5
                    ...
                    //string[] value_keywordargs = ParseString(groupedarglist[0],10,CommaDelim);
                        ...
                        arglist[0].Length > 0 && !char.IsUpper(arglist[0][0]))
                    {
                        ...
                        if (!string.IsNullOrEmpty(singlearglist[1]) && singlearglist[1][0] == '@')
                        {
                            ...
                            if (singlearglist.Length > 1 && singlearglist[1] != null)
                            {
                                ...
                                remainder = singlearglist[1];//string.Join(literalend[0].ToString(), singlearglist, 1, singlearglist.Length - 1);
                            }
                            else
                            {
                                remainder = null;
                    ...
                    #region valuemodKeyword
                    else
                    {
                        int mxlen = value_keywordargs.Length - 1;
                        if (mxlen > 0)
                        {
                            value_keywordargs[0] = value_keywordargs[0].TrimStart('{');//comandi post spawn...
                            value_keywordargs[mxlen] = value_keywordargs[mxlen].TrimEnd('}');
                        }
                        if (valuemodKeywordHash.TryGetValue(value_keywordargs[0], out ValuemodKeyword vmkw) && vmkw != ValuemodKeyword.INVALID)
                        {
                            switch (vmkw)
                            {
                                case ValuemodKeyword.RND:
                                {
                                    ...
                                    if (value_keywordargs.Length > 2)
                                    {
                                        // get a random number
                                        string randvalue = "0";
                                        if (int.TryParse(value_keywordargs[1], out int min) && int.TryParse(value_keywordargs[2], out int max))
                                        {
                                ...
                                case ValuemodKeyword.RNDLIST:
                                case ValuemodKeyword.RNDSTRLIST:
                                {
                                ...
                                case ValuemodKeyword.GET:
                                {
                                    ...
                                    if (value_keywordargs.Length > 2)
                                    {
                                        ...
                                        if (value_keywordargs[1].StartsWith("0x", StringComparison.Ordinal))
                                        {
                                            if (!int.TryParse(value_keywordargs[1].Substring(2), NumberStyles.HexNumber, Core.Culture, out int serial))
                                            {
                                                serial = -1;
                                            }
                                ...
                                case ValuemodKeyword.GETVAR:
                                {
                                    ...
                                    if (value_keywordargs.Length > 1)
                                    {
                                        ...
                                        XmlLocalVariable var = (XmlLocalVariable)XmlAttach.FindAttachment(refobject as IEntity, typeof(XmlLocalVariable), varname);
                                ...
                                case ValuemodKeyword.GETVALUE:
                                {
                                    // syntax is GETVALUE,varname
                                    if (value_keywordargs.Length > 1)
                                    {
                                        string varname = value_keywordargs[1];

                                        // look for the xmlvalue attachment with the given name
                                        XmlValue var = (XmlValue)XmlAttach.FindAttachment(refobject as IEntity, typeof(XmlValue), varname);

                                        if (var != null)
                                        {
                                            string result = SetPropertyValue(spawner, o, arglist[0], var.Value.ToString());
                                        ...
                                        {
                                            status_str = arglist[0] + " : No such value";
                                            no_error = false;
                                        }
                                        if (arglist.Length < 3)
                                        {
                                    ...
                                    {
                                        status_str = "Invalid GETVALUE args : " + arglist[1];
                                        no_error = false;
                                    }
                                    if (arglist.Length < 3)
                                    {
                                ...
                                case ValuemodKeyword.GETONCARRIED:
                                {
                                    ...
                                    if (value_keywordargs.Length > 2)
                                    {
                                        ...
                                        if (trigmob != null && !trigmob.Deleted)
                                        {
                                            ...
                                            if (value_keywordargs.Length > 4)
                                            {
                                                ...
                                                if (value_keywordargs[3].ToLower(Core.Culture) == "equippedonly")
                                                {
                                                    equippedonly = true;
                                                }
                                                else if (!bool.TryParse(value_keywordargs[3], out equippedonly))
                                                {
                                ...
                                case ValuemodKeyword.GETONTRIGMOB:
                                {
                                ...
                                case ValuemodKeyword.GETONNEARBY:
                                {
                                    ...
                                    if (value_keywordargs.Length > 3)
                                    {
                                        ...
                                        if (!int.TryParse(value_keywordargs[1], out int range))
                                        {
                                            range = -1;
                                        }
                                ...
                                case ValuemodKeyword.GETONPARENT:
                                {
                                    ...
                                    {
                                        object parent = null;
                                        if (refobject is Item it)
                                        {
                                            parent = it.Parent;
                                        }
                                        else if (refobject is XmlAttachment xa)
                                        {
                                            parent = xa.AttachedTo;
                                        }
                                ...
                                case ValuemodKeyword.GETONTHIS:
                                {
                                ...
                                case ValuemodKeyword.GETONTAKEN:
                                {
                                ...
                                case ValuemodKeyword.GETONGIVEN:
                                {
                                ...
                                case ValuemodKeyword.GETONSPAWN:
                                {
                                    ...
                                    if (value_keywordargs.Length > 2)
                                    {
                                        ...
                                        //Type ptype;
                                        // get target object
                                        if (!int.TryParse(subgroupstr, out int subgroup))
                                        {
                                            subgroup = -1;
                                        }
                                ...
                                case ValuemodKeyword.GETONSPAWNENTRY:
                                {
                                    //GETONSPAWNENTRY[,spawnername],entrynumber,prop
                                    // find the spawn entry
                                    int entrynum = -1;// = value_keywordargs[1];
                                    string propstr = null;// = value_keywordargs[2];
                                    string spawnerstr = null;
                                    if (value_keywordargs.Length > 2)
                                    {
                                        if (!int.TryParse(value_keywordargs[1], out entrynum))
                                        {
                                            no_error = false;
                                            status_str = "Invalid entry number in GETONSPAWNENTRY";
                                        }
                                        else
                                        {
                                            propstr = value_keywordargs[2];
                                        }
                                    }
                                    else if (value_keywordargs.Length > 3)
                                    {
                                        if (!int.TryParse(value_keywordargs[2], out entrynum))
                                        {
                                            no_error = false;
                                            status_str = "Invalid entry number in GETONSPAWNENTRY";
                                        }
                                    ...
                                    {
                                        no_error = false;
                                        status_str = "GETONSPAWNENTRY: missing arguments";
                                    }
                                    if (no_error)
                                    {
                                        if (spawnerstr != null)
                                        ...
                                        if (spawner != null)
                                        {
                                            ...
                                            {
                                                XmlSpawner.SpawnObject targetobj = spawner.SpawnObjects[i];
                                                // is this references entryindex
                                                if (entrynum == i)
                                                {
                                                    string resultstr = ApplyToProperty(spawner, targetobj, o, propstr, arglist[0]);
                                                    if (resultstr != null)
                                                    {
                                                        status_str = "GETONSPAWNENTRY error: " + resultstr;
                                                        no_error = false;
                                                    }
                                                    i = spawner.SpawnObjects.Count;
                                                }
                                            }
                                        }
                                        else
                                        {
                                            status_str = "Invalid spawner in GETONSPAWNENTRY";
                                            no_error = false;
                                        }
                                    }
                                    if (arglist.Length < 3)
                                ...
                                case ValuemodKeyword.GETACCOUNTTAG:
                                {
                                    ...
                                    if (value_keywordargs.Length > 1)
                                    {
                                        ...
                                        if (trigmob != null && !trigmob.Deleted)
                                        {
                                            if (trigmob.Account is Account acct)
                                            {
                                ...
                                case ValuemodKeyword.MUL:
                                {
                                    ...
                                    if (value_keywordargs.Length > 1)
                                    {
                                        ...
                                        if (value_keywordargs.Length > 2)
                                        {
                                            if (float.TryParse(value_keywordargs[1], NumberStyles.Float, Core.Culture, out float d0) && float.TryParse(value_keywordargs[2], NumberStyles.Float, Core.Culture, out float d1))
                                            {
                                                incvalue = string.Format("{0}", Utility.RandomFloat(d0, d1));
                                            }
                                        ...
                                        // get the current property value
                                        string tmpvalue = GetPropertyValue(spawner, o, arglist[0], out Type ptype);
                                        ...
                                        else
                                        {
                                            ...
                                            try
                                            {
                                                string[] arglist2 = ParseString(tmpvalue, 2, EqualDelim);
                                                string[] arglist3 = ParseString(arglist2[1], 2, SpaceDelim);
                                                currentvalue = arglist3[0].Trim();
                                            }
                                            catch { }
                                            string tmpstr = currentvalue;
                                            if (float.TryParse(currentvalue, NumberStyles.Float, Core.Culture, out float d0) && float.TryParse(incvalue, NumberStyles.Float, Core.Culture, out float d1))
                                            {
                                ...
                                case ValuemodKeyword.AMOUNTCARRIED:
                                {
                                ...
                                case ValuemodKeyword.PLAYERSINRANGE:
                                {
                                    ...
                                    {
                                        IPooledEnumerable<Mobile> ie = ((Item)refobject).GetMobilesInRange(range);
                                    ...
                                    {
                                        IPooledEnumerable<Mobile> ie = ((Mobile)refobject).GetMobilesInRange(range);
                                        foreach (Mobile p in ie)
                                        {
                                            if (p.Player && p.AccessLevel == AccessLevel.Player)
                                            {
                                ...
                                case ValuemodKeyword.TRIGSKILL:
                                {
                                    if (value_keywordargs.Length > 1)
                                    {
                                        if (spawner != null && spawner.TriggerSkill != null)
                                        {
                                            ...
                                            if (value_keywordargs[1].ToLower(Core.Culture) == "name")
                                            {
                                                skillstr = spawner.TriggerSkill.Name;
                                            }
                                            else if (value_keywordargs[1].ToLower(Core.Culture) == "value")
                                            {
                                                skillstr = spawner.TriggerSkill.Value.ToString();
                                            }
                                            else if (value_keywordargs[1].ToLower(Core.Culture) == "cap")
                                            {
                                                skillstr = spawner.TriggerSkill.Cap.ToString();
                                            }
                                            else if (value_keywordargs[1].ToLower(Core.Culture) == "base")
                                            {
                        ...
                        #endregion
                        #region typemodKeyword
                        else if (IstypeModKeyword(spawner, o, trigmob, arglist, out string[] keywordargs, out TypemodKeyword tmkw))//IsTypemodKeyword(keywordargs[0]))
                        {
                            switch (tmkw)
                            {
                                case TypemodKeyword.MUSIC:
                                {
                                ...
                                case TypemodKeyword.PRIVSOUND:
                                {
                                    //
                                    //  PRIVSOUND keyword
                                    //
                                    int sound = -1;
                                    // Syntax PRIVSOUND,soundnumber
                                    // try to get the soundnumber argument
                                    if (keywordargs.Length < 2)
                                    {
                                        status_str = "Missing sound number";
                                    ...
                                    {
                                        Effects.PlaySound((Mobile)o, sound);
                                    }
                                    if (arglist.Length < 2)
                                    {
                                        return no_error;
                                ...
                                case TypemodKeyword.EFFECT:
                                {
                                    ...
                                    if (keywordargs.Length > 3)
                                    {
                                        ...
                                        if (keywordargs.Length > 5)
                                        {
                                            int y = 0;
                                            int z = 0;
                                            if (!int.TryParse(keywordargs[3], out int x) || !int.TryParse(keywordargs[4], out y) || !int.TryParse(keywordargs[5], out z))
                                ...
                                case TypemodKeyword.SEFFECT:
                                {
                                    ...
                                    int hue = 0;
                                    int rendermode = 0;
                                    // syntax is SEFFECT,itemid,duration,speed,hue,rendermode,[x,y,z]
                                    // try to get the effect argument
                                    if (keywordargs.Length < 5)
                                    {
                                        status_str = "Missing numbers";
                                        no_error = false;
                                    }
                                    else
                                    {
                                        ...
                                        if (!int.TryParse(keywordargs[2], out duration))
                                        {
                                            status_str = "Improper duration number format"; no_error = false;
                                        }
                                        if (!int.TryParse(keywordargs[3], out speed))
                                        {
                                            status_str = "Improper speed number format"; no_error = false;
                                        }
                                        if (!int.TryParse(keywordargs[4], out hue))
                                        {
                                            status_str = "Improper hue number format"; no_error = false;
                                        }
                                        if (!int.TryParse(keywordargs[5], out rendermode))
                                        {
                                            status_str = "Improper rendermode number format"; no_error = false;
                                        }
                                    }
                                    // by default just use the spawn location
                                    Point3D eloc;
                                    ...
                                    if (keywordargs.Length > 5)
                                    {
                                        ...
                                        if (keywordargs.Length > 7)
                                        {
                                            int y = 0;
                                            int z = 0;
                                            if (!int.TryParse(keywordargs[6], out int x) || !int.TryParse(keywordargs[7], out y) || !int.TryParse(keywordargs[8], out z))
                                            { status_str = "Improper effect location format"; }
                                            eloc = new Point3D(x, y, z);
                                        }
                                    }
                                    if (effect >= 0 && emap != Map.Internal)
                                    {
                                        Effects.SendLocationEffect(eloc, emap, effect, duration, speed, hue, rendermode);
                                    }
                                    if (arglist.Length < 2)
                                    {
                                        return no_error;
                                ...
                                case TypemodKeyword.BOLTEFFECT:
                                {
                                    //
                                    //  BOLTEFFECT keyword
                                    //
                                ...
                                case TypemodKeyword.DELETE:
                                {
                                    ...
                                    else if (o is Mobile mob)
                                    {
                                        if (!mob.Player)
                                        {
                                            Timer.DelayCall(() => { mob?.Delete(); });
                                        }
                                    }
                                    else if (o is XmlAttachment xatt)
                                    {
                                        Timer.DelayCall(() => { xatt?.Delete(); });
                                    }
                                ...
                                case TypemodKeyword.UNEQUIP:
                                {
                                    ...
                                    if (keywordargs.Length > 2)
                                    {
                                        if (keywordargs[2].ToLower(Core.Culture) == "delete")
                                        {
                                            remove = true;
                                        }
                                        else
                                ...
                                case TypemodKeyword.MSG:
                                {
                                    ...
                                    if (keywordargs.Length > 1)
                                    {
                                        if (!float.TryParse(keywordargs[1], NumberStyles.Float, Core.Culture, out drop_probability))
                                    ...
                                    if (o is Mobile || o is Item)
                                    {
                                        ...
                                        if (Utility.RandomFloat() < drop_probability)
                                        {
                                            if (o is Mobile)
                                            {
                                ...
                                case TypemodKeyword.SENDASCIIMSG:
                                {
                                ...
                                case TypemodKeyword.SAY:
                                {
                                    ...
                                    if (keywordargs.Length > 1)
                                    {
                                        ...
                                        if (keywordargs.Length > 2)
                                        {
                                            if (!bool.TryParse(keywordargs[2], out unhide))
                                            { status_str = "Invalid unhide bool : " + arglist[2]; no_error = false; }
                                        }
                                    }
                                    if (o is Mobile)
                                    {
                                ...
                                case TypemodKeyword.YELL:
                                {
                                    ...
                                    {
                                        if (!float.TryParse(keywordargs[1], NumberStyles.Float, Core.Culture, out drop_probability))
                                        { status_str = "Invalid yell probability : " + arglist[1]; no_error = false; }
                                        if (keywordargs.Length > 2)
                                        {
                                            if (!bool.TryParse(keywordargs[2], out unhide))
                                            { status_str = "Invalid unhide bool : " + arglist[2]; no_error = false; }
                                ...
                                case TypemodKeyword.EMOTE:
                                {
                                    //the mobile, human type, will emote something
                                    //syntax is EMOTE,int
                                    if (o is Mobile m)
                                    {
                                        if (m.Body.IsHuman && keywordargs.Length > 1)
                                        {
                                            if (int.TryParse(keywordargs[1], out int emote) && emote > 0 && emote < 43)
                                            {
                                                new ESound(m, emote);
                                            }
                                        }
                                ...
                                case TypemodKeyword.SETVALUE:
                                {
                                    // syntax is SETVALUE,varname,value[,duration] - it's a wrapper for XmlValue
                                    if (keywordargs.Length > 2)
                                    {
                                        float duration = 0;
                                        int.TryParse(keywordargs[2], out int val);
                                        if (keywordargs.Length > 3)
                                        {
                                            float.TryParse(keywordargs[3], NumberStyles.Float, Core.Culture, out duration);
                                        }
                                        if (o is IEntity ie)
                                        {
                                            XmlValue x = (XmlValue)XmlAttach.FindAttachment(ie, typeof(XmlValue), keywordargs[1]);
                                            if (x != null)
                                            {
                                                x.Value += val;
                                                if (duration > 0)
                                            ...
                                            else
                                            {
                                                if (duration > 0)
                                                {
                                                    XmlAttach.AttachTo(ie, new XmlValue(keywordargs[1], val, duration));
                                                }
                                                else
                                                {
                                                    XmlAttach.AttachTo(ie, new XmlValue(keywordargs[1], val));
                                                }
                                            }
                                        }
                                    }
                                ...
                                case TypemodKeyword.SETVAR:
                                {
                                    // syntax is SETVAR,varname,value[,duration] - it's a wrapper for XmlLocalVariable
                                    if (keywordargs.Length > 2)
                                    {
                                        string val = keywordargs[2];
                                        float duration = 0;
                                        if (keywordargs.Length > 3)
                                        {
                                            float.TryParse(keywordargs[3], NumberStyles.Float, Core.Culture, out duration);
                                        ...
                                        if (o is IEntity ie)
                                        {
                                            XmlLocalVariable x = (XmlLocalVariable)XmlAttach.FindAttachment(ie, typeof(XmlLocalVariable), keywordargs[1]);
                                            if (x != null)
                                            {
                                                x.Data = val;
                                                if (duration > 0)
                                            ...
                                            else
                                            {
                                                if (duration > 0)
                                                {
                                                    XmlAttach.AttachTo(ie, new XmlLocalVariable(keywordargs[1], val, duration));
                                                }
                                                else
                                                {
                                                    XmlAttach.AttachTo(ie, new XmlLocalVariable(keywordargs[1], val));
                                                }
                                            }
                                        }
                                    }
                                ...
                                case TypemodKeyword.FLASH:
                                {
                                    ...
                                    // 5 is black flash
                                    if (o is PlayerMobile m)
                                    {
                                        if (m.NetState != null && m.NetState.Running)
                                        {
                                            if (short.TryParse(keywordargs[1], out short flash) && flash > 0 && flash < 6)
                                            {
                                ...
                                case TypemodKeyword.BCAST:
                                {
                                    ...
                                    if (font >= 0)
                                    {
                                        ...
                                        BroadcastMessage(AccessLevel.Player, hue, font, arglist[1]);
                                    }
                                    else
                                    {
                                        // broadcast a message to all players
                                        BroadcastMessage(AccessLevel.Player, hue, arglist[1]);
                                    }
        ...
        public static PropertyInfo LookupPropertyInfo(Type type, string propname)
        {
            ...
            if (TypeInfoCache.TryGetValue(type, out Dictionary<string, PropertyInfo> infocache))
            {
                infocache.TryGetValue(propname.ToLower(Core.Culture), out pinfo);
                return pinfo;
            }
            else
            {
                // if it cant be found, then do the full cache
                infocache = new Dictionary<string, PropertyInfo>();
                PropertyInfo[] props = type.GetProperties(BindingFlags.Static | BindingFlags.Instance | BindingFlags.Public);
                foreach (PropertyInfo p in props)
                {
                    infocache[p.Name.ToLower(Core.Culture)] = p;
                }

                TypeInfoCache[type] = infocache;
                infocache.TryGetValue(propname.ToLower(Core.Culture), out pinfo);
            }
            return pinfo;
        }

        public static string ParseSingleGetKeyword(XmlSpawner spawner, object o, string valstr, Mobile trigmob)
        {
            ...
            string[] arglist = valstr.Trim().Split(',');
            if (arglist.Length > 1 && IsValueKeyword(arglist[0]))
            {
                return ParseValueKeyword(spawner, o, trigmob, arglist, out _);
            }
            return valstr;
        }
        ...
        public static string ParseForKeywords(XmlSpawner spawner, object o, string valstr, Mobile trigmob, bool literal, out Type ptype)
        {
            ...
            // additemstr should have the full list of args <arg2/arg3/arg4>/arg5 if they are there.  In the case of /arg1/ADD/arg2
            // it will just have arg2
            string[] groupedarglist = ParseString(str, 2, SquareLeftDelim);
            ...
            //string[] arglist = ParseString(groupedarglist[0],4,CommaDelim);
            ...
            // or a bool
            else if ((str.ToLower(Core.Culture)) == "true" || (str.ToLower(Core.Culture) == "false"))
            {
                ptype = typeof(bool);
                return str;
            }
            ...
            {
                return ParseValueKeyword(spawner, o, trigmob, arglist, out ptype);
            }
            else if (literal)
            {
                ptype = typeof(string);
        ...
        public static string ParseValueKeyword(XmlSpawner spawner, object o, Mobile trigmob, string[] arglist, out Type ptype)
        {
            ptype = null;
            if (valueKeywordHash.TryGetValue(arglist[0], out ValueKeyword kw))
            {
                switch (kw)
                {
                    case ValueKeyword.GETONMOB when arglist.Length > 2:
                    {
                        // syntax is GETONMOB,mobname[,mobtype],property
                    ...
                    case ValueKeyword.GET when arglist.Length > 2:
                    {
                        // syntax is GET,[itemname -OR- SETITEM][,itemtype],property
                        ...
                        // is the itemname a serialno?
                        object testitem = null;
                        if (arglist[1].StartsWith("0x", StringComparison.Ordinal))
                        {
                            if (!int.TryParse(arglist[1].Substring(2), NumberStyles.HexNumber, Core.Culture, out int serial))
                            {
                                serial = -1;
                            }
                        ...
                        else if (arglist[1] == "SETITEM")
                        {
                            if (spawner != null && !spawner.Deleted && spawner.SetItem != null)
                            {
                                testitem = spawner.SetItem;
                            }
                        }
                    ...
                    case ValueKeyword.GETONCARRIED when arglist.Length > 2:
                    {
                        // syntax is GETONCARRIED,itemname[,itemtype][,equippedonly],property
                        ...
                        {
                            propname = arglist[4];
                            if (arglist[3].ToLower(Core.Culture) == "equippedonly")
                            {
                                equippedonly = true;
                            }
                            else
                        ...
                        else//testitem is NULL, check at least if the type is VALID
                        {
                    ...
                    case ValueKeyword.GETONNEARBY when arglist.Length > 3:
                    {
                        // syntax is GETONNEARBY,range,name[,type][,searchcontainers],property
                        // or GETONNEARBY,range,name[,type][,searchcontainers],[ATTACHMENT,type,name,property]
                        ...
                        string typestr = null;
                        bool searchcontainers = false;
                        if (int.TryParse(arglist[1], out int range) && range >= 0)
                        {
                            if (arglist.Length > 4)
                            {
                                typestr = arglist[3];
                            ...
                            {
                                string getvalue = GetPropertyValue(spawner, nearbylist[0], propname, out ptype);
                                return ParseGetValue(getvalue, ptype);
                            }
                        }
                        return null;
                    }
                    case ValueKeyword.GETONTRIGMOB when arglist.Length > 1:
                    {
                        // syntax is GETONTRIGMOB,property
                    ...
                    case ValueKeyword.GETVAR when arglist.Length > 1:
                    {
                        // syntax is GETVAR,varname
                        string varname = arglist[1];

                        if (o is XmlAttachment att)
                        {
                            o = att.AttachedTo;
                        }

                        // look for the xmllocalvariable attachment with the given name
                        if (XmlAttach.FindAttachment(o as IEntity, typeof(XmlLocalVariable), varname) is XmlLocalVariable var)
                        {
                            return var.Data;
                        }
                        else
                    ...
                    case ValueKeyword.GETVALUE when arglist.Length > 1:
                    {
                        // syntax is GETVALUE,varname
                        string varname = arglist[1];
                        ...
                        // look for the xmllocalvariable attachment with the given name
                        if (XmlAttach.FindAttachment(o as IEntity, typeof(XmlValue), varname) is XmlValue var)
                        {
                    ...
                    case ValueKeyword.GETONPARENT when arglist.Length > 1:
                    {
                        // syntax is GETONPARENT,property
                        ...
                        if (o is Item item)
                        {
                            getvalue = GetPropertyValue(spawner, item.Parent, arglist[1], out ptype);
                        }
                        else if (o is XmlAttachment att)
                        {

                            getvalue = GetPropertyValue(spawner, att.AttachedTo, arglist[1], out ptype);
                        }

                        return ParseGetValue(getvalue, ptype);
                    }
                    case ValueKeyword.GETONGIVEN when arglist.Length > 1:
                    {
                        // syntax is GETONGIVEN,property
                        Item taken = null;
                        if (o is XmlAttachment att)
                        {
                            taken = GetGiven(att.AttachedTo);
                        }
                        else
                        {
                            taken = GetGiven(o);
                    ...
                    case ValueKeyword.GETONTAKEN when arglist.Length > 1:
                    {
                        // syntax is GETONTAKEN,property
                        Item taken = null;
                        if (o is XmlAttachment att)
                        {
                            taken = GetTaken(att.AttachedTo);
                        }
                        else
                        {
                            taken = GetTaken(o);
                    ...
                    case ValueKeyword.GETONTHIS when arglist.Length > 1:
                    {
                        // syntax is GETONTHIS,property
                    ...
                    case ValueKeyword.GETONSPAWN when arglist.Length > 2:
                    {
                    ...
                    case ValueKeyword.GETONSPAWNENTRY when arglist.Length > 2:
                    {
                        //GETONSPAWNENTRY[,spawnername],entrynum,prop
                        // find the spawn entry
                        int entrynum = -1;// = value_keywordargs[1];
                        string propstr = null;// = value_keywordargs[2];
                        string spawnerstr = null;
                        if (arglist.Length > 3)
                        {
                            if (int.TryParse(arglist[2], out entrynum))
                            {
                                spawnerstr = arglist[1];
                                propstr = arglist[3];
                            }
                        ...
                        {
                            if (int.TryParse(arglist[1], out entrynum))
                            {
                                propstr = arglist[2];
                            }
                            else
                        ...
                        if (spawner != null)
                        {
                            ...
                            {
                                XmlSpawner.SpawnObject targetobj = spawner.SpawnObjects[i];
                                // is this references by entryindex
                                if (entrynum == i)
                                {
                                    if (targetobj == null)
                                    {
                    ...
                    case ValueKeyword.GETACCOUNTTAG when arglist.Length > 1:
                    {
                        // syntax is GETACCOUNTTAG,tagname
                        ptype = typeof(string);
                    ...
                    case ValueKeyword.RND when arglist.Length > 2:
                    {
                        arglist[1] = ParseSingleGetKeyword(spawner, o, arglist[1], trigmob);
                        arglist[2] = ParseSingleGetKeyword(spawner, o, arglist[2], trigmob);
                        // syntax is RND,min,max
                        string randvalue = "0";
                        ptype = typeof(int);
                        if (int.TryParse(arglist[1], out int min) && int.TryParse(arglist[2], out int max))
                        {
                            randvalue = string.Format("{0}", Utility.RandomMinMax(min, max));
                        }
                    ...
                    case ValueKeyword.RNDLIST when arglist.Length > 1:
                    {
                        ...
                        arglist[randindex] = ParseSingleGetKeyword(spawner, o, arglist[randindex], trigmob);

                        // return the list entry as the value

                        return arglist[randindex];
                    }
                    case ValueKeyword.RNDSTRLIST when arglist.Length > 1:
                    {
                    ...
                    case ValueKeyword.AMOUNTCARRIED when arglist.Length > 1:
                    {
                        // syntax is AMOUNTCARRIED,itemtype[,banksearch[,itemname]]
                        ...
                        if (typestr != null)
                        {
                            ...
                            if (arglist.Length > 2)
                            {
                                if (!bool.TryParse(arglist[2], out banksearch) && arglist[2].ToLower(Core.Culture) != "banksearch")
                                {
                                    return null;
                                }
                    ...
                    case ValueKeyword.PLAYERSINRANGE when arglist.Length > 1:
                    {
                        // syntax is PLAYERSINRANGE,range
                        ...
                        int nplayers = 0;
                        // get the number of players in range
                        int.TryParse(arglist[1], out int range);
                        ...
                        {
                            IPooledEnumerable<Mobile> ie = it.GetMobilesInRange(range);
                            foreach (Mobile p in ie)
                            {
                                if (p.Player && p.AccessLevel == AccessLevel.Player)
                                {
                        ...
                        {
                            IPooledEnumerable<Mobile> ie = m.GetMobilesInRange(range);
                            foreach (Mobile p in ie)
                            {
                                if (p.Player && p.AccessLevel == AccessLevel.Player)
                                {
                    ...
                    case ValueKeyword.TRIGSKILL when arglist.Length > 1:
                    {
                        if (spawner != null && spawner.TriggerSkill != null)
                        {
                            // syntax is TRIGSKILL,name|value|cap|base
                            if (arglist[1].ToLower(Core.Culture) == "name")
                            {
                                ptype = typeof(string);
                                return spawner.TriggerSkill.Name;
                            }
                            else if (arglist[1].ToLower(Core.Culture) == "value")
                            {
                                ptype = typeof(float);
                                return spawner.TriggerSkill.Value.ToString();
                            }
                            else if (arglist[1].ToLower(Core.Culture) == "cap")
                            {
                                ptype = typeof(float);
                                return spawner.TriggerSkill.Cap.ToString();
                            }
                            else if (arglist[1].ToLower(Core.Culture) == "base")
                            {
                                ptype = typeof(float);
                    ...
                    case ValueKeyword.RANDNAME when arglist.Length > 1:
                    {
                        // syntax is RANDNAME,nametype
                        if (arglist[1].ToLower(Core.Culture) == "auto" && trigmob != null)
                        {
                            return NameList.RandomName(trigmob.Female);
                        }
        ...
        public static string ParseGetValue(string str, Type ptype)
        {
            ...
            // find the separator
            string[] arglist = str.Split(EqualDelim, 2);
            ...
            if (arglist.Length > 1)
            {
                if (IsNumeric(ptype))
                {
                    ...
                    string[] arglist2 = arglist[1].Trim().Split(SpaceDelim, 2);
        ...
        public static bool CheckPropertyString(XmlSpawner spawner, object o, string testString, Mobile trigmob, out string status_str)
        {
            ...
            string[] arglist = ParseString(testString, 2, AndOrDelim);
            if (arglist.Length < 2)
            {
            ...
            else
            {
                ...
                bool first = CheckSingleProperty(spawner, o, arglist[0], trigmob, out _);
        ...
        public static bool CheckSingleProperty(XmlSpawner spawner, object o, string testString, Mobile trigmob, out string status_str)
        {
            ...
            if (o == null || string.IsNullOrEmpty(testString))
            {
                return false;
            }
            ...
            string[] arglist = ParseString(testString, 2, ComparerDelim);
            if (arglist.Length < 2)
            {
            ...
            string value1 = ParseForKeywords(spawner, o, arglist[0].Trim(), trigmob, false, out Type ptype1);
            ...
            string value2 = ParseForKeywords(spawner, o, arglist[1].Trim(), trigmob, false, out Type ptype2);
            ...
            int base1 = 10;
            int base2 = 10;
            if (IsNumeric(ptype1) && !string.IsNullOrEmpty(value1) && value1.StartsWith("0x", StringComparison.Ordinal))
            {
                base1 = 16;
            }

            if (IsNumeric(ptype2) && !string.IsNullOrEmpty(value2) && value2.StartsWith("0x", StringComparison.Ordinal))
            {
                base2 = 16;
            }
            ...
            if (ptype2 == typeof(TimeSpan) || ptype1 == typeof(TimeSpan))
            {
                if (hasequal)
                {
                    if (TimeSpan.TryParse(value1, out TimeSpan ts1) && TimeSpan.TryParse(value2, out TimeSpan ts2))
                    {
                        if (ts1 == ts2)
                        {
                            return !invertreturn;
            ...
            else if (ptype2 == typeof(DateTime) || ptype1 == typeof(DateTime))
            {
                if (hasequal)
                {
                    if (DateTime.TryParse(value1, out DateTime dt1) && DateTime.TryParse(value2, out DateTime dt2))
                    {
                        if (dt1 == dt2)
                        {
                            return !invertreturn;
            ...
            else if ((ptype2 == typeof(float)) && IsNumeric(ptype1))
            {
                if (hasequal)
                {
                    try
                    {
                        if (Convert.ToInt64(value1, base1) == float.Parse(value2))
                        {
                            return !invertreturn;
                        }
                    }
            ...
            else if ((ptype1 == typeof(float)) && IsNumeric(ptype2))
            {
                if (hasequal)
                {
                    try
                    {
                        if (float.Parse(value1) == Convert.ToInt64(value2, base2))
                        {
                            return !invertreturn;
                        }
                    }
            ...
            else if ((ptype1 == typeof(float)) && (ptype2 == typeof(float)))
            {
                float val1 = 0, val2 = 0;
                if (hasequal)
                {
                    if (float.TryParse(value1, NumberStyles.Float, Core.Culture, out val1) && float.TryParse(value2, NumberStyles.Float, Core.Culture, out val2))
                    {
                        if (val1 == val2)
                        {
                            return !invertreturn;
            ...
            else if (ptype2 == typeof(float) || ptype2 == typeof(float))
            {
                float val1 = 0, val2 = 0;
                if (hasequal)
                {
                    ...
                    else { status_str = "invalid float comparison : {0}" + testString; }
                }
                else if (hasnotequals)
                {
                    if (float.TryParse(value1, NumberStyles.Float, Core.Culture, out val1) && float.TryParse(value2, NumberStyles.Float, Core.Culture, out val2))
        ...
        private static void GetItemsIn(Item source, string targetname, Type targettype, string typestr, ref List<object> nearbylist, string proptest)
        {
            if (source != null && source.Items != null && nearbylist != null)
            {
                foreach (Item i in source.Items)
                {
                    ...
                    {
                        if (proptest == null || CheckPropertyString(null, i, proptest, null, out string status_str))
                        {
                            nearbylist.Add(i);
                        }
                    }
        ...
        {
            IPooledEnumerable<Item> itemlist = null;
            IPooledEnumerable<Mobile> mobilelist = null;
            ...
            if (targettype == null || targettype == typeof(Item) || targettype.IsSubclassOf(typeof(Item)))
            {
                ...
                if (itemlist != null)
                {
                    foreach (Item i in itemlist)
                    {
                        ...
                        else if (!i.Deleted && CheckNameMatch(targetname, i.Name) && (typestr == null || (itemtype != null && targettype != null && (itemtype.Equals(targettype) || itemtype.IsSubclassOf(targettype)))))
                        {
        ...
        public static Item SearchMobileForItem(Mobile m, string targetName, string typeStr, bool searchbank, bool equippedonly = false)
        {
            if (m != null && !m.Deleted)
            {
                ...
                for (int i = 0; i < packlist.Count; ++i)
                {
                    ...
                    // dont search bank boxes and don't search in wareboxes!
                    if (item is WareBox || (item is BankBox && (!searchbank || equippedonly)))
                    {
                        continue;
                    }
                    ...
                    {
                        if (item is Container cont && !equippedonly)
                        {
                            Item itemTarget = SearchPackForItem(cont, targetName, typeStr);
        ...
        public static List<Item> SearchMobileForItems(Mobile m, string targetName, string typeStr, bool searchbank, bool equippedonly)
        {
            ...
            if (m != null && !m.Deleted)
            {
                ...
                for (int i = 0; i < packlist.Count; ++i)
                {
                    ...
                    // dont search bank boxes and don't search in wares!
                    if (item is WareBox || (item is BankBox && (!searchbank || equippedonly)))
                    {
                        continue;
                    }
                    ...
                    if (item != null && !item.Deleted)
                    {
                        if (item is Container cont && !equippedonly)
                        {
                            itemlist.AddRange(SearchPackForItems(cont, targetName, typeStr));
                        }
                        // test the item name against the trigger string
                        // if a typestring has been specified then check against that as well
            ...
            if (pack != null && !pack.Deleted && targetName != null && targetName.Length > 0)
            {
                ...
                for (int i = 0; i < packlist.Count; ++i)
                {
                    ...
                    if (item != null && !item.Deleted)
                    {
                        ...
                        {
                            Item itemTarget = SearchPackForItemType(cont, targetName);
        ...
        public static Item SearchMobileForItemType(Mobile m, string targetName, bool searchbank)
        {
            if (m != null && !m.Deleted && targetName != null && targetName.Length > 0)
            {
                ...
                for (int i = 0; i < packlist.Count; ++i)
                {
                    ...
                    // dont search bank boxes and don't search in wares!
                    if (item is WareBox || (item is BankBox && !searchbank))
                    {
                        continue;
                    }
        ...
        public static List<Item> SearchPackListForItemType(Container pack, string targetName, List<Item> itemlist)
        {
            if (pack != null && !pack.Deleted && targetName != null && targetName.Length > 0)
            {
                ...
                {
                    Item item = packlist[i];
                    if (item != null && !item.Deleted && item is Container cont)
                    {
                        itemlist = SearchPackListForItemType(cont, targetName, itemlist);
                    }
                    // test the item name against the trigger string
        ...
        public static bool CheckForNotCarried(Mobile m, string objectivestr)
        {
            ...
            string[] arglist = ParseString(objectivestr, 2, AndOrDelim);
            if (arglist.Length < 2)
            {
        ...
        public static bool SingleCheckForNotCarried(Mobile m, string objectivestr)
        {
            ...
            string[] objstr = ParseString(objectivestr, 8, CommaDelim);
            ...
            if (testitem != null)
            {
                ...
                if (testitem is IXmlQuest token && token.IsValid)
                {
                    if (objstr.Length > objoffset)
                    {
                        ...
                        {
                            if (int.TryParse(objstr[n], out int x))
                            {
        ...
        public static bool SingleCheckForCarried(Mobile m, string objectivestr)
        {
            ...
            while (objoffset < objstr.Length)
            {
                if (objstr[objoffset] != null && objstr[objoffset].Length > 0)
                {
                    ...
                    else if (objstr[objoffset].ToLower(Core.Culture) == "equipped")
                    {
                        equippedonly = true;
                    }
                    else
        ...
        public static Item FindItemByName(XmlSpawner fromspawner, string name, string typestr)
        {
            ...
            name = name.ToLower(Core.Culture);
            ...
            if (targettype != null)
            {
                XmlSpawner.RecentItemNameTypeSearch.TryGetValue((name, targettype), out founditem);
            }
            else
            {
                XmlSpawner.RecentItemNameSearch.TryGetValue(name, out founditem);
            }

            if (founditem != null && !founditem.Deleted)
            {
                return founditem;
            }
            ...
            if (founditem != null)
            {
                ...
                if (targettype != null)
                {
                    XmlSpawner.RecentItemNameTypeSearch[(name, targettype)] = founditem;
                }
                else
                {
                    XmlSpawner.RecentItemNameSearch[name] = founditem;
                }
            }
        ...
        public static Mobile FindMobileByName(XmlSpawner fromspawner, string name, string typestr)
        {
            ...
            if (targettype != null)
            {
                XmlSpawner.RecentMobileNameTypeSearch.TryGetValue((name, targettype), out foundmobile);
            }
            else
            {
                XmlSpawner.RecentMobileNameSearch.TryGetValue(name, out foundmobile);
            }

            if (foundmobile != null && !foundmobile.Deleted)
            {
                return foundmobile;
            }
            ...
            if (foundmobile != null)
            {
                if (targettype != null)
                {
                    XmlSpawner.RecentMobileNameTypeSearch[(name, targettype)] = foundmobile;
                }
                else
                {
                    XmlSpawner.RecentMobileNameSearch[name] = foundmobile;
                }
            }
            return foundmobile;
        }
        ...
        public static XmlSpawner FindSpawnerByName(XmlSpawner fromspawner, string name)
        {
            ...
            {
                name = name.ToLower(Core.Culture);
                XmlSpawner.RecentSpawnerSearch.TryGetValue(name, out XmlSpawner foundspawner);
                if (foundspawner != null && !foundspawner.Deleted)
                {
                    return foundspawner;
                }
                ...
                {
                    XmlSpawner.RecentSpawnerSearch[name] = foundspawner;
                }
                return foundspawner;
            }
        }
            ...
            object refobject, out string remainder, out string status_str)
        {
            ...
            if (keywordargs.Length > 1)
            {
                ...
                try { drop_probability = Convert.ToSingle(keywordargs[1], Core.Culture); }
            ...
            //string[] probargs = ParseString(arglist[0], 2, CommaDelim);
            ...
            if (addattachstr.IndexOf("<") == 0)
            {
                ...
                // it will just have arg2
                string[] attacharglist = ParseString(addattachstr, 2, OpenBracketDelim);
            ...
            if (type != null && type.IsSubclassOf(typeof(XmlAttachment)))
            {
                ...
                if (newo is XmlAttachment)
                {
                    ...
                    if (!XmlAttach.AttachTo(spawner, o as IEntity, attachment))
                    {
            ...
            if (o is Mobile || (o is Container))
            {
                ...
                if (pack != null)
                {
                    ...
                    if (additemstr.IndexOf("<") == 0)
                    {
                        ...
                        // additemstr should have the full list of args <arg2/arg3/arg4>/arg5 if they are there.  In the case of /arg1/ADD/arg2
                        // it will just have arg2
                        string[] itemarglist = ParseString(additemstr, 2, OpenBracketDelim);
                        ...
                        // and get the first part of the string without the >  so itemargs[0] should be arg2/ag3/arg4
                        if (itemargs[0].IndexOf('}') != -1)//post spawn props
                        {
                            itemargstring = ApplySubstitution(spawner, o, trigmob, itemargs[0], true);
                        }
                        else
                        {
                    ...
                    #region itemKeyword
                    if (baseitemtype != null && itemKeywordHash.TryGetValue(baseitemtype, out ItemKeyword ikw) && ikw != ItemKeyword.INVALID)//IsSpecialItemKeyword(baseitemtype))
                    {
                        ...
                        switch (ikw)
                        {
                            ...
                            case ItemKeyword.SCROLL:
                            {
                                ...
                                if (itemkeywordargs.Length == 3)
                                {
                                    if (!int.TryParse(itemkeywordargs[1], out int minCircle))
                                    {
                                        status_str = "Invalid SCROLL args : " + itemtypestr;
                                        return false;
                                    }
                                    if (!int.TryParse(itemkeywordargs[2], out int maxCircle))
                                    {
                                        status_str = "Invalid SCROLL args : " + itemtypestr;
                                        return false;
                                    }
                                    ...
                                    int min = (circle - 1) * 8;
                                    Item item = Loot.RandomScroll(min, min + 7, School.Regular);
                                    if (item != null)
                                    {
                                        pack.DropItem(item);
                                        // could call applyobjectstringproperties on a nested propertylist here to set item attributes
                            ...
                            case ItemKeyword.NECROSCROLL:
                            {
                                ...
                                if (itemkeywordargs.Length == 2)
                                {
                                    if (!int.TryParse(itemkeywordargs[1], out int index))
                                    {
                            ...
                            case ItemKeyword.LOOTPACK:
                            {
                                ...
                                if (itemkeywordargs.Length == 2)
                                {
                                    ...
                                    if (loottype.ToLower(Core.Culture) == "poor")
                                    {
                                        lootpack = LootPack.Poverissimo;
                                    }
                                    else if (loottype.ToLower(Core.Culture) == "meager")
                                    {
                                        lootpack = LootPack.Povero;
                                    }
                                    else if (loottype.ToLower(Core.Culture) == "average")
                                    {
                                        lootpack = LootPack.Normale;
                                    }
                                    else if (loottype.ToLower(Core.Culture) == "rich")
                                    {
                                        lootpack = LootPack.Medio;
                                    }
                                    else if (loottype.ToLower(Core.Culture) == "filthyrich")
                                    {
                                        lootpack = LootPack.Ricco;
                                    }
                                    else if (loottype.ToLower(Core.Culture) == "ultrarich")
                                    {
                                        lootpack = LootPack.Ricchissimo;
                                    }
                                    else if (loottype.ToLower(Core.Culture) == "superboss")
                                    {
                                        lootpack = LootPack.Leggendario;
                                    }
                                    else
                                    {
        ...
        public static string ApplySubstitution(XmlSpawner spawner, object o, Mobile trigmob, string typeName, bool postmod = false)
        {
            // go through the string looking for instances of {keyword} - if ^ is present we leave everything as it is, except for the ^ character
            ...
            while (remaining != null && remaining.Length > 0)
            {
                ...
                {
                    if (startindex + 3 <= remaining.Length && remaining[startindex + 1] == '^')
                    {
                        post = true;
                    }
                }
                ...
                int endindex = remaining.Substring(startindex + sum).IndexOf('}');
                ...
                sb.Append(firstpart);
                string keypart = remaining.Substring(startindex + sum, endindex);

                // try to evaluate and then substitute the arg

                if (!post)//ciò che non viene posticipato viene elaborato SUBITO
                {
                    keypart = ParseForKeywords(spawner, o, keypart.Trim(), trigmob, true, out _);
                ...
                else//ciò che è da posticipare viene riassemblato con i delimitatori
                {
                    sb.Append('{');
                    sb.Append(keypart);
                    sb.Append('}');
                ...
                if (endindex + startindex + sum >= remaining.Length)
                {
                    break;
                }

                remaining = remaining.Substring(endindex + startindex + sum, remaining.Length - endindex - startindex - sum);
            }
            return sb.ToString();
        }
        ...
        public static string[] ParseString(string str, int nitems, char[] delims)
        {
            ...
            string[] args = str.Trim().Split(delims, nitems);
        ...
        private static bool IstypeModKeyword(XmlSpawner spawner, object o, Mobile trigmob, string[] args, out string[] keywordargs, out TypemodKeyword tmkw)
        {
            if (args[0].IndexOf('}') != -1)
            {
                args[0] = ApplySubstitution(spawner, o, trigmob, args[0], true);
            }

            keywordargs = args[0].Trim().Split(',');
            return typemodKeywordHash.TryGetValue(keywordargs[0], out tmkw) && tmkw != TypemodKeyword.INVALID;
        }
        ...
        {
            BroadcastMessage(ac, hue, 3, message);
        }

        public static void BroadcastMessage(AccessLevel ac, int hue, int font, string message)
        {
            Packet p = null;
            if (int.TryParse(message, out int number) && number > 0)
            {
                p = new MessageLocalized(Serial.MinusOne, -1, MessageType.Regular, hue, font, number, "System", string.Empty);
            }
            else
            {
                p = new UnicodeMessage(Serial.MinusOne, -1, MessageType.Regular, hue, font, "ENU", "System", message);
            }
        ...
        public static void PublicOverheadMobileMessage(Mobile mob, MessageType type, int hue, int font, string text, bool noLineOfSight)
        {
            if (mob != null && mob.Map != null)
            {
                ...
                IPooledEnumerable<NetState> eable = mob.Map.GetClientsInRange(mob.Location);
        ...
        public static void PublicOverheadItemMessage(Item item, MessageType type, int hue, int font, string text)
        {
            if (item != null && item.Map != null)
            {
                ...
                IPooledEnumerable<NetState> eable = item.Map.GetClientsInRange(worldLoc, item.GetMaxUpdateRange());
        ...
        public static Item GetTaken(object o)
        {
            ...
            if (XmlAttach.FindAttachment(o as IEntity, typeof(XmlSaveItem), "Taken") is XmlSaveItem si)
            {
                return si.SavedItem;
            }
        ...
        {
            // find the XmlSaveItem attachment
            if (XmlAttach.FindAttachment(o as IEntity, typeof(XmlSaveItem), "Given") is XmlSaveItem si)
            {
                return si.SavedItem;
            }
        ...
        public static void SendMusicToPlayers(string arglist, Mobile triggermob, object refobject, out string status_str)
        {
            ...
            string[] musicstr = ParseString(arglist, 3, CommaDelim);
            int range = 0;
            ...
            if ((range > 0) || (triggermob != null && !triggermob.Deleted))
            {
                ...
                if (range > 0)
                {
                    IPooledEnumerable<NetState> rangelist = null;
        ...
        public static void ResurrectPlayers(string arglist, Mobile triggermob, object refobject, out string status_str)
        {
            ...
            string[] str = ParseString(arglist, 3, CommaDelim);
            int range = 0;
            bool petres = false;
            ...
            {
                // get the range arg
                if (str[2].ToLower(Core.Culture) == "pets")
                {
                    petres = true;
                }
                else
            ...
            try
            {
                if ((range > 0) || (triggermob != null && !triggermob.Deleted))
                {
                    ...
                    if (range > 0)
                    {
                        IPooledEnumerable<Mobile> rangelist = null;
                        if (refitem != null && !refitem.Deleted)
                        {
                        ...
                        if (rangelist != null)
                        {
                            foreach (Mobile p in rangelist)
                            {
                                ...
                                else if (petres && p is BaseCreature bc && bc.ControlMaster == triggermob && bc.Controlled && bc.IsDeadPet)
                                {
                                    bc.ResurrectPet();
                                }
                            }
        ...
        public static void ApplyPoisonToPlayers(string arglist, Mobile triggermob, object refobject, out string status_str)
        {
            ...
            // look for the other args
            string[] str = ParseString(arglist, 4, CommaDelim);
            ...
            if (str.Length > 3)
            {
                if (str[3].ToLower(Core.Culture) == "playeronly")
                {
                    playeronly = true;
                }
                else
        ...
        public static void ApplyDamageToPlayers(string arglist, Mobile triggermob, object refobject, out string status_str)
        {
            ...
            string[] str = ParseString(arglist, 9, CommaDelim);
            bool playeronly = false;
            int range = -1;
            ...
            if (str.Length > 8)
            {
                if (str[8].ToLower(Core.Culture) == "playeronly")
                {
                    playeronly = true;
                }
                else
            ...
            damage = (int)(damage * Utility.c_BilanciaRess);
            try
            {
                if (range >= 0 || (triggermob != null && !triggermob.Deleted))
                {
                    ...
                    if (range >= 0)
                    {
                        ...
                        if (rangelist != null)
                        {
                            foreach (Mobile p in rangelist)
                            {
                                ...
                                if (p is PlayerMobile || !playeronly)
                                {
                                    AOS.Damage(p, damage, (int)(phys * Utility.c_RevBilanciaRess), fire, cold, pois, ener);
                                }
                            }
                            rangelist.Free();
                        }
        ...
        public static void ExecuteAction(Mobile trigmob, object attachedto, string action)
        {
            ...
            {
                SpawnTypeKeyword(attachedto, TheSpawn, typeName, substitutedtypeName, true, trigmob, loc, map, out _);
            }
            else
            {
                ...
                try
                {
                    string[] arglist = ParseString(substitutedtypeName, 3, SlashDelim);
    ...
    string propertyString, out string status_str)
		...
		*/
            ...
            List<XmlSpawner.SpawnPositionInfo> spawnpositioning, Mobile triggermob, Point3D location, Map map, XmlGumpCallback gumpcallback, out string status_str, byte loops)
        {
            ...
            #region typeKeyword
            if (typeKeywordHash.TryGetValue(typeName, out TypeKeyword tkw) && tkw != TypeKeyword.INVALID)//IsTypeKeyword(typeName))
            {
                ...
                switch (tkw)
                {
                    case TypeKeyword.SET:
                    {
                        ...
                        string[] keywordargs = ParseString(arglist[0], 3, CommaDelim);
                        ...
                        if (keywordargs.Length > 1)
                        {
                            ...
                            // is the itemname a serialno?
                            object setitem = null;
                            if (keywordargs[1].StartsWith("0x", StringComparison.Ordinal))
                            {
                                if (int.TryParse(keywordargs[1].Substring(2), NumberStyles.HexNumber, Core.Culture, out int serial) && serial > 0)
                                {
                    ...
                    case TypeKeyword.SETONTHIS:
                    {
                        ...
                        if (arglist.Length > 0)
                        {
                            string[] objstr = ParseString(arglist[0], 2, CommaDelim);
                            if (objstr.Length > 1)
                            {
                                proptest = objstr[1];
                            }
                    ...
                    {
                        // the syntax is FOREACH,objecttype[,name][,range/regionname]/action
                        string[] arglist = ParseSlashArgs(substitutedtypeName, 2);
                        if (arglist.Length > 1)
                        {
                            string[] objstr = ParseString(arglist[0], 4, CommaDelim);
                            if (objstr.Length < 2)
                            {
                            ...
                            if (objecttype != null)
                            {
                                ...
                                if (objstr.Length > 2)
                                {
                                    ...
                                    if (objstr.Length > 3)
                                    {
                                        if (!int.TryParse(objstr[3], out range))
                                        {
                                            range = -1;
                                            regname = objstr[3];
                                        }
                                ...
                                if (!userange)
                                {
                                    ...
                                    {
                                        if (invoker is IEntity entity && entity.Map != null && entity.Map != Map.Internal)
                                        {
                                            if (!entity.Map.Regions.TryGetValue(regname, out reg) || reg == null)
                                            {
                                                status_str = "Invalid region name or range in FOREACH";
                                            }

                                            return false;
                                        }
                                    }
                                    else if (spawner == null || spawner.Deleted)
                                    {
                                        status_str = "Invalid range definition (no spawner) in FOREACH";
                                        return false;
                                    }
                                }
                                ...
                                if (typeof(Mobile).IsAssignableFrom(objecttype))
                                {
                                    ...
                                    if (!userange)
                                    {
                                        if (reg != null)
                                        {
                                            foreach (Mobile m in reg.GetMobiles())
                                            {
                                ...
                                else if (typeof(Item).IsAssignableFrom(objecttype))
                                {
                                    ...
                                    if (!userange)
                                    {
                                        if (reg != null)
                                        {
                                            foreach (Item i in reg.GetItems())
                                            {
                                        ...
                                        else if (spawner != null)
                                        {
                                            if (spawner.SpawnRegion != null && spawner.HasRegionPoints(spawner.SpawnRegion))
                                            {
                                                foreach (Item i in spawner.SpawnRegion.GetItems())
                                                {
                                                    if (objecttype.IsAssignableFrom(i.GetType()) && CheckNameMatch(objectname, i.Name))
                                                    {
                                                        items.Add(i);
                    ...
                    {
                        // the syntax is SETVAR,varname,value[,duration]

                        string[] arglist = ParseSlashArgs(substitutedtypeName, 1);
                        ...
                        if (arglist.Length > 0)
                        {
                            ...
                            else if (objstr.Length < 3)
                            {
                                status_str = "missing value in SETVAR";
                                return false;
                            }
                            ...
                            if (objstr.Length > 3)
                            {
                                float.TryParse(objstr[3], NumberStyles.Float, Core.Culture, out duration);
                            }

                            string varname = objstr[1];
                            string varval = objstr[2];

                            // find the xmllocalvariable attachment with that name
                            if (invoker is IEntity ie)
                            {
                                XmlLocalVariable a = (XmlLocalVariable)XmlAttach.FindAttachment(ie, typeof(XmlLocalVariable), varname);
                                if (a == null)
                                {
                                    // doesnt already exist so add it
                                    if (duration > 0)
                                    {
                                        XmlAttach.AttachTo(ie, new XmlLocalVariable(varname, varval, duration));
                                    }
                                    else
                                    {
                                        XmlAttach.AttachTo(ie, new XmlLocalVariable(varname, varval));
                                    }
                                }
                                else
                                {
                                    ...
                                    if (duration > 0)
                                    {
                                        a.Expiration = TimeSpan.FromMinutes(duration);
                                    }
                                }
                            }
                        }
                    ...
                    case TypeKeyword.SETVALUE:
                    {
                        // syntax is SETVALUE,varname,value[,duration] - it's a wrapper for xmlvalue
                        // in case of attachment with same name present the value is summed to old value
                        ...
                        if (arglist.Length > 0)
                        {
                            ...
                            if (objstr.Length < 2)
                            {
                                status_str = "missing varname in SETVALUE";
                                return false;
                            }
                            else if (objstr.Length < 3)
                            {
                                status_str = "missing VALUE in SETVALUE";
                                return false;
                            }
                            ...
                            string varname = objstr[1];
                            int.TryParse(objstr[2], out int val);
                            if (invoker is IEntity ie)
                            {
                                XmlValue x = (XmlValue)XmlAttach.FindAttachment(ie, typeof(XmlValue), varname);
                                if (x != null)
                                {
                                    x.Value += val;
                                    if (duration > 0)
                                ...
                                else
                                {
                                    if (duration > 0)
                                    {
                                        XmlAttach.AttachTo(ie, new XmlValue(varname, val, duration));
                                    }
                                    else
                                    {
                                        XmlAttach.AttachTo(ie, new XmlValue(varname, val));
                                    }
                                }
                            }
                        }
                        ...
                        {
                            status_str = "no value assigned to SETVALUE";
                            return false;
                        }
                    ...
                    case TypeKeyword.SETONNEARBY:
                    {
                        ...
                        if (arglist.Length > 0)
                        {
                            string[] objstr = ParseString(arglist[0], 6, CommaDelim);
                            if (objstr.Length < 3)
                            {
                    ...
                    case TypeKeyword.SETONPETS:
                    {
                        ...
                        if (arglist.Length > 0)
                        {
                            string[] objstr = ParseString(arglist[0], 3, CommaDelim);
                            if (objstr.Length < 2)
                            {
                        ...
                        foreach (object nearbyobj in nearbylist)
                        {
                            ...
                            if (nearbyobj is BaseCreature pet && pet.Controlled && pet.ControlMaster == triggermob)
                            {
                                ApplyObjectStringProperties(spawner, substitutedtypeName, nearbyobj, triggermob, invoker, out status_str);
                            }
                        }
                    ...
                    case TypeKeyword.SETONCARRIED:
                    {
                        ...
                        if (arglist.Length > 0)
                        {
                            ...
                            if (objstr.Length > 3)
                            {
                                if (objstr[3].ToLower(Core.Culture) == "equippedonly")
                                {
                                    equippedonly = true;
                                }
                                else
                    ...
                    case TypeKeyword.SETONSPAWNENTRY:
                    {
                        ...
                        for (int i = 0; i < targetspawner.SpawnObjects.Count; ++i)
                        {
                    ...
                    case TypeKeyword.SETONPARENT:
                    {
                        ...
                        {
                            ApplyObjectStringProperties(spawner, substitutedtypeName, ((XmlAttachment)invoker).AttachedTo, triggermob, invoker, out status_str);
                        }
                    ...
                    case TypeKeyword.TAKEGIVE:
                    {
                        ...
                        string[] keywordargs = ParseString(arglist[0], 4, CommaDelim);
                        string[] givekeywordargs = ParseString(givelist[0], 2, CommaDelim);
                        int quantity = 0;
                        bool banksearch = false;
                        bool success = false;
                        ...
                        if (itemTarget != null)
                        {
                            ...
                            if (success)
                            {
                                AddItemToTarget(spawner, triggermob, givekeywordargs, givelist, triggermob, invoker, false, out _, out status_str);
                            }
                        }
                        TheSpawn.SpawnedObjects.Add(new KeywordTag(substitutedtypeName, spawner));
                        break;
                    ...
                    {
                        //syntax is GIVE[,probabilità (0.01=1% 1=100%)]/itemtypetogive
                        string[] arglist = ParseSlashArgs(substitutedtypeName, 3);
                        ...
                        if (arglist.Length > 1)
                        {
                            ...
                            // note this will be an arg to some property
                            string[] keywordargs = ParseString(arglist[0], 2, CommaDelim);
                            AddItemToTarget(spawner, triggermob, keywordargs, arglist, triggermob, invoker, false, out _, out status_str);
                    ...
                    case TypeKeyword.TAKE:
                    {
                        ...
                        string[] keywordargs = ParseString(arglist[0], 5, CommaDelim);
                        float drop_probability = 1;
                        int quantity = 0;
                        bool banksearch = false;
                        Item savedItem = null;
                        ...
                        if (drop_probability == 1 || Utility.RandomFloat() < drop_probability)
                        {
                            // search the trigger mob for the named item
                            Item itemTarget = SearchMobileForItem(triggermob, targetName, typestr, banksearch);
                            ...
                            if (itemTarget != null)
                            {
                                ...
                                if (invoker is IEntity ie)
                                {
                                    XmlSaveItem si = (XmlSaveItem)XmlAttach.FindAttachment(ie, typeof(XmlSaveItem), "Taken");
                                    if (si == null)
                                    {
                                        XmlAttach.AttachTo(ie, new XmlSaveItem("Taken", savedItem, triggermob));
                                    }
                                    else
                                    {
                    ...
                    case TypeKeyword.TAKEBYTYPE:
                    {
                        ...
                        if (drop_probability == 1 || Utility.RandomFloat() < drop_probability)
                        {
                            ...
                            if (invoker is IEntity ie)
                            {
                                if (!(XmlAttach.FindAttachment(ie, typeof(XmlSaveItem), "Taken") is XmlSaveItem si))
                                {
                                    XmlAttach.AttachTo(ie, new XmlSaveItem("Taken", savedItem, triggermob));
                                }
                                else
                    ...
                    case TypeKeyword.GUMP:
                    {
                        ...
                        // if constructorname is KEYWORDS then the responses will be threated as xmlspawner KEYWORDS (will override original CALLBACK!)
                        string[] arglist = ParseSlashArgs(substitutedtypeName, 2);
                        string gumpText;
                        bool ovd = false;
                        if (arglist.Length > 1)
                        ...
                        string[] gumpkeywordargs = ParseString(arglist[0], 4, CommaDelim);
                        string gumpTitle = "";
                        ...
                        {
                            if (gumpkeywordargs[3] == "KEYWORDS")
                            {
                                ovd = (gumpNumber != 2 && gumpNumber != 5);//permettere l'uso di un reply scritto significa esporre tutto xmlspawner al pubblico, un bel cazzo per il culo va...
                            }
                            else
                            {
                    ...
                    case TypeKeyword.WAITUNTIL:
                    {
                        ...
                        if (arglist.Length > 0)
                        {
                            ...
                            if (keywordargs.Length > 1)
                            {
                                if (!float.TryParse(keywordargs[1], NumberStyles.Float, Core.Culture, out delay))
                                {
                            ...
                            if (keywordargs.Length > 2)
                            {
                                if (!float.TryParse(keywordargs[2], NumberStyles.Float, Core.Culture, out timeout))
                                {
                    ...
                    case TypeKeyword.GOTOIF:
                    {
                        // the syntax is GOTOIF/condition/thengroup [/elsegroup]
                        string[] arglist = ParseSlashArgs(substitutedtypeName, 5);
                        string condition = null;
                        int thengroup = -1;
                        int elsegroup = -1;
                        if (arglist.Length < 3)
                        {
                            status_str = "insufficient args to GOTOIF";
                        }
                        else
                        {
                            condition = arglist[1];
                            if (!int.TryParse(arglist[2], out thengroup))
                            {
                                status_str = "invalid thengroup arg to GOTOIF";
                                thengroup = -1;
                            }
                        }
                        if (arglist.Length > 3)
                        {
                            if (!int.TryParse(arglist[3], out elsegroup))
                            {
                                status_str = "invalid elsegroup arg to GOTOIF";
                                elsegroup = -1;
                            }
                        }
                    ...
                    case TypeKeyword.MEFFECT:
                    {
                        ...
                        if (arglist.Length > 0)
                        {
                            string[] keywordargs = ParseString(arglist[0], 9, CommaDelim);
                            ...
                            else
                            {
                                ...
                                if (!int.TryParse(keywordargs[1], out int effect))
                                {
                                    status_str = "Improper effect number format";
                                    effect = -1;
                                }

                                if (!int.TryParse(keywordargs[2], out int speed))
                                {
                                ...
                                Point3D eloc1 = new Point3D(x, y, z);
                                ...
                                Point3D eloc2 = new Point3D(x, y, z);
                    ...
                    case TypeKeyword.EFFECT:
                    {
                        ...
                        if (arglist.Length > 0)
                        {
                            string[] keywordargs = ParseString(arglist[0], 6, CommaDelim);
                            int effect = -1;
                            int duration = 1;
                            ...
                            if (keywordargs.Length > 3)
                            {
                                ...
                                if (keywordargs.Length > 5)
                                {
                                    if (!int.TryParse(keywordargs[3], out int x) || !int.TryParse(keywordargs[4], out int y) || !int.TryParse(keywordargs[5], out int z))
                                    {
                                        status_str = "Improper effect location format";
                                ...
                                else if (keywordargs[3].ToLower(Core.Culture) == "trigmob" && triggermob != null)
                                {
                                    eloc = triggermob.Location;
                                    emap = triggermob.Map;
                                }
                            }
                            if (status_str != null)
                    ...
                    case TypeKeyword.SEFFECT:
                    {
                        ...
                        if (arglist.Length > 0)
                        {
                            ...
                            int hue = 0;
                            int rendermode = 0;
                            // syntax is SEFFECT,itemid,duration,speed,hue,rendermode[,x,y,z] or EFFECT,itemid,duration,speed,hue,rendermode[,trigmob]
                            // try to get the effect argument
                            // some interesting effects are explosion(14013,15), sparkle(14155,15), explosion2(14000,13)
                            if (keywordargs.Length < 6)
                            {
                                status_str = "Missing effect number, duration, speed, hue or rendermode";
                            }
                            else
                            {
                                if (!int.TryParse(keywordargs[1], out effect))
                                ...
                                if (!int.TryParse(keywordargs[4], out hue))
                                {
                                    status_str = "Improper effect hue format";
                                    hue = 0;
                                }
                                if (!int.TryParse(keywordargs[5], out rendermode))
                                {
                                    status_str = "Improper render mode format";
                                    rendermode = 0;
                                }
                            }
                            // by default just use the spawner location
                            ...
                            if (keywordargs.Length > 6)
                            {
                                ...
                                if (keywordargs.Length > 8)
                                {
                                    if (!int.TryParse(keywordargs[6], out int x) || !int.TryParse(keywordargs[7], out int y) || !int.TryParse(keywordargs[8], out int z))
                                    {
                                        status_str = "Improper effect location format";
                                        x = spawner.Location.X;
                                        y = spawner.Location.Y;
                                ...
                                else if (keywordargs[6].ToLower(Core.Culture) == "trigmob" && triggermob != null)
                                {
                                    eloc = triggermob.Location;
                                    emap = triggermob.Map;
                                }
                    ...
                    case TypeKeyword.CAST:
                    {
                        ...
                        BaseWand cwand = new PlaceHolderWand
                        {
                            Parent = caster
                        };
                        ...
                        if (spell != null)
                        {
                            ...
                            try
                            {
                                ...
                                if (spelltype != null && ((spelltargetmethod = spelltype.GetMethod("Target")) != null || (spelltargetmethod = spelltype.GetMethod("OnCast")) != null))
                                {
                    ...
                    case TypeKeyword.BCAST:
                    {
                        ...
                        if (arglist.Length > 1)
                        {
                            ...
                            if (!string.IsNullOrEmpty(arglist[1]) && arglist[1][0] == '@')
                            {
                                arglist = ParseSlashArgs(substitutedtypeName, 2);
                            ...
                            {
                                // broadcast an ascii message to all players
                                BroadcastMessage(AccessLevel.Player, hue, font, msg);
                            }
                            else
                            {
                                // standard unicode message format
                                BroadcastMessage(AccessLevel.Player, hue, msg);
                            }
            ...
            #endregion
            #region itemKeyword
            else if (itemKeywordHash.TryGetValue(typeName, out ItemKeyword ikw))//IsSpecialItemKeyword(typeName))
            {
                ...
                // itemtypestr will have the form keyword[,x[,y]]
                string[] itemkeywordargs = ParseString(itemtypestr, 3, CommaDelim);
                // deal with the special keywords
                switch (ikw)
                {
                    case ItemKeyword.ARMOR:
                    {
                        ...
                        if (itemkeywordargs.Length == 3)
                        {
                            if (!int.TryParse(itemkeywordargs[1], out int min) || !int.TryParse(itemkeywordargs[2], out int max))
                            {
                    ...
                    case ItemKeyword.SCROLL:
                    {
                        ...
                        if (itemkeywordargs.Length == 3)
                        {
                            if (!int.TryParse(itemkeywordargs[1], out int minCircle) || !int.TryParse(itemkeywordargs[2], out int maxCircle))
                            {
                                status_str = "Invalid SCROLL args : " + itemtypestr;
                                return false;
                            }
                    ...
                    case ItemKeyword.NECROSCROLL:
                    {
                        ...
                        if (itemkeywordargs.Length == 2)
                        {
                            if (!int.TryParse(itemkeywordargs[1], out int necroindex))
                            {
                                status_str = "Invalid NECROSCROLL args : " + itemtypestr;
                                return false;
                            }
        ...
        private static XmlGumpCallback KeywordCB_Instance = new XmlGumpCallback(XmlUse.KeywordGumpCallback);
        #endregion
    }
}