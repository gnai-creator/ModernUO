...
namespace Server.Items
{
    ...
    public class MageCrystal : BaseSocketAugmentation, ISocketCrystal
    {
        //Cristallo della Magia
        //Armi: +Spell Channeling<BR>Scudi: +Spell Channeling
        public override int LabelNumber => 504313;
        [Constructable]
        public MageCrystal() : base(0xF8E)
        {
            Hue = 8;
        }

        public MageCrystal(Serial serial) : base(serial)
        {
        }
        ...
        public override bool OnAugment(Mobile from, object target)
        {
            ...
            else if (target is ISpellChanneling)
            {
                ((ISpellChanneling)target).SpellChanneling = true;
            }
            else
            {
                return false;
        ...
        {
            return (target is BaseWeapon || target is BaseShield || target is ISpellChanneling);
        }
        ...
        public override bool OnRecover(Mobile from, object target, int version)
        {
            ...
            {
                ((ISpellChanneling)target).SpellChanneling = false;
            }
            else
            {
                return false;
