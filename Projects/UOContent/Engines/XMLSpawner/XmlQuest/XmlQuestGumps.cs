...
namespace Server.Gumps
{
    public class XmlSimpleGump : Gump
    {
        ...
        public override object Invoker
        {
            get
            {
                if (m_invoker is XmlAttachment att && att.AttachedTo != null)
                {
                    string s = att.AttachedTo.ToString();
                    int begin = s.IndexOf('"');
        ...
        private void LocalAddHtml(string text, int x, int y, int width, int height, int color, bool background, bool scrollbar)
        {
            if (text == null)
            {
                return;
            ...
            if (text.StartsWith("#"))
            {
                if (int.TryParse(text.Substring(1, text.Length - 1), out int cliloc))
                {
            ...
            {
                AddHtml(x, y, width, height, XmlSimpleGump.Color(text, string.Format("{0:X}", color)), background, scrollbar);
            }
        }
        ...
        public XmlSimpleGump(object invoker, string gumptext, string gumptitle, int gumptype, BaseXmlSpawner.KeywordTag tag, Mobile from, XmlGumpCallback gumpcallback) : base(0, 0)
        {
            ...
            if (gumptype == 4)
            {
                ...
                height = height + nselections * 35;//40
            }
            if (gumptype == 5)
            {
                maintext = ParseGumpText(gumptext);
                //gumpSelections.Count;
            ...
            { // multiple selection type gump
              // parse the gump text to get the selections and responses
                int lenght = (maintext.Length > 100 ? 182 : 91);
                LocalAddHtml(maintext, 105, 159, 299, lenght, 0xEFEF5A, false, true);

                for (int i = 0; i < gumpSelections.Count; ++i)
                {
                    int y = 178 + lenght + i * 35;//360 + i*40;
            ...
            else if (gumptype == 5)
            {
                ...
                for (int i = 0; i < gumpSelections.Count; ++i)
                {
                    ...
                    if (args.Length > 1)
                    {
                        ...
                        if (args[0].ToLower(Core.Culture) == "button")
                        {
                            // syntax is button,gumpid,x,y
                            if (args.Length > 3)
                            {
                                if (args[1].StartsWith("0x", StringComparison.Ordinal))
                                {
                            ...
                            if (args[0].ToLower(Core.Culture) == "label")
                        {
                            ...
                            if (args[0].ToLower(Core.Culture) == "html")
                        {
                            ...
                            if (args.Length > 5)
                            {
                                ...
                                if (args.Length > 6 && args[5].StartsWith("0x", StringComparison.Ordinal) && args[5].Trim().Length == 8)
                                {
                            ...
                            if (args[0].ToLower(Core.Culture) == "textentry")
                        {
                            ...
                            if (args[0].ToLower(Core.Culture) == "radio")
                        {
                            int gumpid1 = 0;
                            int gumpid2 = 0;
                            ...
                            if (args[0].ToLower(Core.Culture) == "image")
                        {
                            ...
                            if (args[0].ToLower(Core.Culture) == "imagetiled")
                        {
                            ...
                            if (args[0].ToLower(Core.Culture) == "item")
                        {
