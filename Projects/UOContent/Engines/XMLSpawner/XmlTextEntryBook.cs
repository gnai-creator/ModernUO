...
namespace Server.Items
{
    ...
    public class XmlTextEntryBook : BaseEntryBook
    {
        ...
        public override float DefaultWeight => 0f;
    ...
    public class BaseEntryBook : Item
    {
        ...
        public static void ContentChange(NetState state, PacketReader pvSrc)
        {
            ...
            if (bookitem is BaseBook)
            {
                ...
                BaseContentChange((BaseBook)bookitem, state, pvSrc);
                return;
            }

            // then try it as a text entry book

            if (!(bookitem is BaseEntryBook book))
            {
                return;
            }
            ...
            if (book is XmlTextEntryBook tebook)
            {
                tebook.m_bookcallback?.Invoke(state.Mobile, tebook.m_args, sb.ToString());
            }
        }
        ...
        public static void BaseContentChange(BaseBook book, NetState state, PacketReader pvSrc)
        {
            ...
            if (!book.Writable || !from.InRange(book.GetWorldLocation(), 1) || !book.IsAccessibleTo(from))
            {
                return;
            }
