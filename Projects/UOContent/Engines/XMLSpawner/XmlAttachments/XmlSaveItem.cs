...
namespace Server.Engines.XmlSpawner2
{
    public class XmlSaveItem : XmlAttachment
    {
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public Mobile WasOwnedBy { get => m_WasOwnedBy; set => m_WasOwnedBy = value; }
        ...
        public XmlSaveItem(string name, Mobile wasownedby)
        {
            Name = name;
        ...
        public override LogEntry OnIdentify(Mobile from)
        {
            ...
            {
                return new LogEntry(LocalizerDef(), string.Format("{2}: Item {0} expires in {1} mins", SavedItem, Expiration.TotalMinutes, Name));
            }
            else
            {
                return new LogEntry(LocalizerDef(), string.Format("{1}: Item {0}", SavedItem, Name));
            }
        }
    }
}
