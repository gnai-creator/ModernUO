...
namespace Server.Engines.XmlSpawner2
{
    public class XmlData : XmlAttachment
    {
        ...
        public string Data { get => m_DataValue; set => m_DataValue = value; }

        public override string AttachedBy => m_DataValue;
        ...
        public override LogEntry OnIdentify(Mobile from)
        {
            ...
            {
                return new LogEntry(LocalizerDef(), string.Format("{2}: Data {0} expires in {1} mins", Data, Expiration.TotalMinutes, Name));
            }
            else
            {
                return new LogEntry(LocalizerDef(), string.Format("{1}: Data {0}", Data, Name));
            }
        }
    }
}
