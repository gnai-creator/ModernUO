...
namespace Server.Engines.XmlSpawner2
{
    public class XmlDate : XmlAttachment
    {
        ...
        public DateTime Date { get => m_DataValue; set => m_DataValue = value; }
        ...
        public override LogEntry OnIdentify(Mobile from)
        {
            ...
            {
                return new LogEntry(LocalizerDef(), string.Format("{2}: Date {0} expires in {1} mins", Date, Expiration.TotalMinutes, Name));
            }
            else
            {
                return new LogEntry(LocalizerDef(), string.Format("{1}: Date {0}", Date, Name));
            }
        }
    }
}
