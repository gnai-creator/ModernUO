...
namespace Server.Engines.XmlSpawner2
{
    public class XmlMorph : XmlAttachment
    {
        ...
        private TimeSpan m_Duration = TimeSpan.FromSeconds(120.0);    // default 30 second duration
        private MorphTimer m_MorphTimer;
        private DateTime m_MorphEnd;

        [CommandProperty(AccessLevel.GameMaster)]
        public int MorphID { get => m_MorphID; set => m_MorphID = value; }
        ...
        public override LogEntry OnIdentify(Mobile from)
        {
            ...
            {
                msg = string.Format("Morph in {0} ends in {1} mins", m_MorphID, Expiration.TotalMinutes);
            }
            else
            {
                msg = string.Format("Morph in {0} duration {1} min", m_MorphID, m_Duration.TotalMinutes);
            }

            if (ActivationWord != null)
            {
                return new LogEntry(LocalizerDef(), string.Format("{0} activated by '{1}'", msg, ActivationWord));
            }
            else
            {
