    ...
    public class XmlDialog : XmlAttachment
    {
        ...
        public TimeSpan GameTOD
        {
            get
            {
                ...
                {
                    map = AttachedTo.Map;
                    x = AttachedTo.Location.X;
                    y = AttachedTo.Location.Y;
                }

                Server.Items.Clock.GetTime(map, x, y, out int hours, out int minutes);
        ...
        public bool CheckDependsOn(SpeechEntry s, int id)
        {
            ...
            {
                if (int.TryParse(args[i].Trim(), out int check) && check == id)
                {
                    return true;
                }
            }
        ...
        internal override string EntryString
        {
            get
            {
                if (m_SpeechEntries == null)
                    return null;
                StringBuilder sb = new StringBuilder();
                foreach (SpeechEntry entry in m_SpeechEntries)
                {
        ...
        private SpeechEntry FindMatchingKeyword(Mobile from, string keyword, int currententryid)
        {
            ...
            foreach (SpeechEntry s in m_SpeechEntries)
            {
                    ...
                    || (CheckDependsOn(s, currententryid) && (IsActive || currententryid == 0)))
                {
                    ...
                    if (s.Condition != null)
                    {
                        if (!BaseXmlSpawner.CheckPropertyString(null, this, s.Condition, from, out string error))
                        {
                            status_str = error;
                            continue;
                        }
                    ...
                    if ((keyword == null && s.Keywords == null))
                    {
                        ...
                        if (keyword != null && s.Keywords != null)
                    {
                        string[] arglist = s.Keywords.Split(',', StringSplitOptions.RemoveEmptyEntries);
                        for (int i = 0; i < arglist.Length; ++i)
                        {
                            if (arglist[i] == "$TRIGNAME")
                            {
                                string[] tocheck = from.Name.ToLower(Core.Culture).Split(' ', StringSplitOptions.RemoveEmptyEntries);
                                string result = string.Empty;

                                for (int x = 0; x < tocheck.Length; ++x)
                                {
                                    if (keyword.ToLower(Core.Culture).Contains(tocheck[x]))
                                    {
                                        result = tocheck[x];
                                    }
                                }
                            ...
                            else if (arglist[i].Contains("$GOTO"))
                            {
                                ...
                                {
                                    if (int.TryParse(tocheck.Trim(), out int togo))
                                    {
        ...
        public static SpeechEntry CopyEntry(SpeechEntry s)
        {
            ...
            {
                Action = s.Action,
                AllowNPCTrigger = s.AllowNPCTrigger,
                Condition = s.Condition,
                DependsOn = s.DependsOn,
                EntryNumber = s.EntryNumber,
                Gump = s.Gump,
                ID = s.ID,
                IgnoreCarried = s.IgnoreCarried,
                Keywords = s.Keywords,
                LockConversation = s.LockConversation,
                Pause = s.Pause,
                PrePause = s.PrePause,
                SpeechHue = s.SpeechHue,
        ...
        private static XmlGumpCallback DialogCB_Instance = new XmlGumpCallback(DialogGumpCallback);
        ...
        private void ExecuteGump(Mobile mob, string gumpstring)
        {
            ...
            Map map = null;
            if (AttachedTo != null && AttachedTo.Parent == null)
            {
                loc = AttachedTo.Location;
                map = AttachedTo.Map;
            }

            if (typeName == "GUMP")
            {
                BaseXmlSpawner.SpawnTypeKeyword(this, TheSpawn, typeName, substitutedtypeName, true, mob, loc, map, DialogCB_Instance, out string error, 0);
        ...
        private void ExecuteAction(Mobile mob, string action)
        {
            ...
            {
                BaseXmlSpawner.SpawnTypeKeyword(AttachedTo, TheSpawn, typeName, substitutedtypeName, true, mob, loc, map, out string error);
                status_str = error;
            }
            else
            {
        ...
        public override void OnSpeech(SpeechEventArgs e)
        {
            ...
            if (e.Mobile == AttachedTo || e.Mobile.AccessLevel > TriggerAccessLevel || (e.Mobile.Hidden && e.Mobile.Alive))
            {
                e.Handled = false;
                return;
            }
        ...
        private void CheckForReset()
        {
            ...
            if (!m_HoldProcessing &&
                ...
                (AttachedTo != null && m_ActivePlayer != null && !IsInRange(m_ActivePlayer, AttachedTo, ResetRange))))
            {
                Reset();
            }
        ...
        private void DelayedSpeech((SpeechEntry, Mobile) state)
        {
            SpeechEntry matchentry = state.Item1;
            Mobile m = state.Item2;
            if (matchentry != null && m != null && !m.Deleted && m.Account != null)
            {
                CurrentEntry = matchentry;
                string text = CurrentEntry.Text;
                if (m.Account.AutoTranslate && text != null && m.AccessLevel == AccessLevel.Player)
                {
                    Task.Run(() => ThreadedEntryDelayed(m, text, true));
                }
                else
                {
                    ThreadedEntryDelayed(m, text, false);
                }
            }
            else
            {
        ...
        private void ThreadedEntryDelayed(Mobile m, string text, bool translate)
        {
            if (text != null)
            {
                if (translate)
                {
                    ...
                    text = BaseTranslate.TranslateStaticText(text, dir);
                }
                text = BaseXmlSpawner.ApplySubstitution(null, this, m, text);
                ...
                if (AttachedTo is Item it)
                {
                    ...
                    it.PublicOverheadMessage(MessageType.Regular, speechhue, false, text);
                }
                else if (AttachedTo is Mobile mob)
                {
        ...
        private class SaveNPCTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (XmlAttach.FindAttachment(targeted as IEntity, typeof(XmlDialog)) is XmlDialog xa)
                {
        ...
        private class LoadNPCTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is IEntity ie)
                {
                    if (XmlAttach.FindAttachment(ie, typeof(XmlDialog)) is XmlDialog xa)
                    {
