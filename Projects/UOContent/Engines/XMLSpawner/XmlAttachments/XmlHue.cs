...
namespace Server.Engines.XmlSpawner2
{
    public class XmlHue : XmlAttachment
    {
        ...
        public override LogEntry OnIdentify(Mobile from)
        {
            ...
            {
                return new LogEntry(LocalizerDef(), string.Format("Hue {0} expires in {1} mins", m_Hue, Expiration.TotalMinutes));
            }
            else
            {
                return new LogEntry(LocalizerDef(), string.Format("Hue {0}", m_Hue));
            }
        }
