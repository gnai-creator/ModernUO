...
namespace Server.Engines.XmlSpawner2
{
    public class XmlMessage : XmlAttachment
    {
        ...
        public string Message { get => m_MessageStr; set => m_MessageStr = value; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int Range { get => proximityrange; set => proximityrange = value; }

        [CommandProperty(AccessLevel.GameMaster)]
        public string ActivationWord { get => m_Word; set => m_Word = value; }

        [CommandProperty(AccessLevel.GameMaster)]
        public int Charges { get => m_Charges; set => m_Charges = value; }
        ...
        {
            if (from == null || from.AccessLevel == AccessLevel.Player || ActivationWord == null)
            {
                return null;
            }
            ...
            if (Charges > 0)
            {
                return new LogEntry(LocalizerDef(), string.Format("{0} - {1} secs between uses, {2} charges left : trigger on '{3}'", Message, Refractory.TotalSeconds, Charges, ActivationWord));
            }
            else
            {
                return new LogEntry(LocalizerDef(), string.Format("{0} - {1} secs between uses : trigger on '{2}'", Message, Refractory.TotalSeconds, ActivationWord));
            }
        }
        ...
        public override void OnTrigger(object activator, Mobile m)
        {
            ...
            {
                ((Item)AttachedTo).PublicOverheadMessage(MessageType.Regular, 0x3B2, false, Message);
            }
            else
                if (AttachedTo is Mobile)
            {
                ((Mobile)AttachedTo).PublicOverheadMessage(MessageType.Regular, 0x3B2, false, Message);
            }
