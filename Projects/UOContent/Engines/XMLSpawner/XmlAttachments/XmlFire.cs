...
namespace Server.Engines.XmlSpawner2
{
    public class XmlFire : XmlAttachment
    {
        ...
        public XmlFire(int damage, double refractory, double expiresin, string name)
        {
            m_Damage = damage;
            Expiration = TimeSpan.FromMinutes(expiresin);
            Refractory = TimeSpan.FromSeconds(refractory);
        ...
        [Attachable]
        public XmlFire(int damage, double refractory, double expiresin, int weaponuses)
        {
            m_Damage = damage;
            Expiration = TimeSpan.FromMinutes(expiresin);
            Refractory = TimeSpan.FromSeconds(refractory);
        ...
        public XmlFire(int damage, double refractory, double percent, bool _)
        {
            m_Damage = damage;
            Refractory = TimeSpan.FromSeconds(refractory);
            m_Percent = percent;
        ...
        public override void OnWeaponHit(Mobile attacker, Mobile defender, BaseWeapon weapon, ref int damageGiven, int originalDamage)
        {
            ...
            {
                bool isweapon = AttachedTo is BaseWeapon;
                damage = (int)(damage * Utility.c_BilanciaRess);//bilancia la ress
                attacker.MovingParticles(defender, (isweapon ? 0x36F4 : 0x36D4), 7, 0, false, true, 9502, 4019, 0x160);
        ...
        public override void OnMovement(MovementEventArgs e)
        {
            ...
            if (AttachedTo is Item item && item.Parent == null && Utility.InRange(e.Mobile.Location, item.GetWorldLocation(), proximityrange))
            {
        ...
        {
            //1005150 -> Danno da fuoco
            if (Expiration > TimeSpan.Zero)
            {
                if (Refractory > TimeSpan.Zero)//~1_val~ ~2_val~ finisce in ~3_val~ min : ~4_val~ sec tra ogni uso
                {
                    return new LogEntry(LocalizerA(1), string.Format("#{0}\t{1}\t{2:F2}\t{3:F1}", 1005150, m_Damage, Expiration.TotalMinutes, m_Refractory.TotalSeconds));
                }
                else//~1_val~ ~2_val~ finisce in ~3_val~ min
                {
                    return new LogEntry(LocalizerA(2), string.Format("#{0}\t{1}\t{2:F2}", 1005150, m_Damage, Expiration.TotalMinutes));
                }
            }
            else
            {
                ...
                {
                    return new LogEntry(LocalizerA(3), string.Format("#{0}\t{1}\t{2:F1}", 1005150, m_Damage, m_Refractory.TotalSeconds));
                }
                else//~1_val~ ~2_val~
                {
                    return new LogEntry(LocalizerA(4), string.Format("#{0}\t{1}", 1005150, m_Damage));
                }
            }
        }
