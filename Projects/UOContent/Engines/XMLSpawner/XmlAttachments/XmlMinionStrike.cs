...
namespace Server.Engines.XmlSpawner2
{
    public class XmlMinionStrike : XmlAttachment
    {
        ...
        private List<BaseCreature> MinionList = new List<BaseCreature>();
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public string Minion { get => m_Minion; set => m_Minion = value; }
        ...
        public override LogEntry OnIdentify(Mobile from)
        {
            ...
            {
                msg = string.Format("Summons : {0} {1}% chance ends in {2} mins", m_Minion, Chance, Expiration.TotalMinutes);
            }
            else
            {
                msg = string.Format("Summons : {0}", m_Minion);
            }

            if (Refractory > TimeSpan.Zero)
            {
                return new LogEntry(LocalizerDef(), string.Format("{0} : {1} sec between uses", msg, Refractory.TotalSeconds));
            }
            else
            {
