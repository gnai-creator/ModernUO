...
namespace Server.Mobiles
{
    public class XmlGetAttGump : Gump
    {
        ...
        private IEntity m_TargetObject;
        ...
        private bool TestAge(object o)
        {
            ...
            if (o is XmlAttachment)
            {
                ...
                if (Searchagedirection)
                {
                    ...
                    if ((DateTime.UtcNow - a.CreationTime) > TimeSpan.FromHours(Searchage))
                    {
                        return true;
                    }
                }
        ...
        private List<XmlAttachment> Search(IEntity target, out string status_str)
        {
            status_str = null;
            ...
            if (attachments != null)
            {
                foreach (XmlAttachment i in attachments)
                {
                    ...
                    // check for name
                    if (Dosearchname && (i.Name != null) && (Searchname != null) && (i.Name.ToLower(Core.Culture).IndexOf(Searchname.ToLower(Core.Culture)) >= 0))
                    {
                        hasname = true;
                    }
        ...
        [Usage("XmlGetAtt")]
        [Description("Trova gli attachment in un oggetto selezionato e mostra il gump di quell'oggetto con tutti gli attachment")]
        public static void XmlGetAtt_OnCommand(CommandEventArgs e)
        {
            e.Mobile.Target = new GetAttachTarget(e);
        }

        public XmlGetAttGump(Mobile from, IEntity targeted, int x, int y) : this(from, targeted, true, false,
        false, false, false,
        null, null, false, 0,
        null, -1, 0,
        false, false,
        ...
        public XmlGetAttGump(Mobile from, IEntity targeted, bool firststart, bool descend,
                    ...
                    if (info.ButtonID >= 5000 && info.ButtonID < 5000 + MaxEntries)
                    {
                        ...
                        if (m_SelectionList != null && i >= 0 && i < m_SelectionList.Length && !SelectAll)
                        {
                            ...
                            if (m_SearchList != null && (m_SearchList.Count - DisplayFrom > i))
                            {
                                ...
                                if (a != null)
                                {
                                    LogEntry le = a.OnIdentify(state.Mobile);
                                    if (le != null)
                                    {
                                        state.Mobile.SendLocalizedMessage(le.Number, le.Args);
                                    }
                                }
                            }
                        }
        ...
        public class XmlConfirmDeleteGump : Gump
        {
            ...
            public XmlConfirmDeleteGump(Mobile from, IEntity target, List<XmlAttachment> searchlist, bool[] selectedlist, int displayfrom, bool selectall, int allcount) : base(0, 0)
            {
